...
Public Class TplnPolygonSet
   ...
   End Sub
   Public Sub SetDataInsertBlock(hsGush As HashSet(Of Integer), ByRef tShapeFOData As FDO_Manager.ShapeFOData, tFilterList As DMCommon.dmList)
      If miMapTheme = DMAcadExt.enMapTheme.Parcels Then

         SetDataInsertBlock_Parcel(hsGush, tShapeFOData, tFilterList)
            ...
            If oResBuffer IsNot Nothing Then
               ...
               If mdicParcelData.TryGetValue(iPgonIDNew, tParcelData) Then
                  If iIndex = 0 Then
                     ...
                     mtShapeFOData.Gush = New GushData(0, tParcelData.Block, tParcelData.BlockAdd, tParcelData.Status, 0, 0.0)
                  End If
                  miTopoIDCounter += 1
                  oXDataParcel.ID = miTopoIDCounter
                  oXDataParcel.DataID = iPgonIDNew
   ...
   Public Sub SetDataInsertBlock_Parcel(setGush As HashSet(Of Integer), ByRef tShapeFOData As FDO_Manager.ShapeFOData, tFilterList As DMCommon.dmList)

      If setGush IsNot Nothing Then
         DMCommon.Debug.MsgBox("08_400c", setGush.Count, DMCommon.Debug.GetList(setGush))
      End If
      Dim oEditor As Autodesk.AutoCAD.EditorInput.Editor = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor
      Dim oMapApplication As Autodesk.Gis.Map.MapApplication = Autodesk.Gis.Map.HostMapApplicationServices.Application
      Dim oMapUtility As Autodesk.Gis.Map.MapUtility = oMapApplication.ActiveProject.MapUtility
      ...
      Dim tParcelData As ParcelData = Nothing
      Dim oResBuffer As Autodesk.AutoCAD.DatabaseServices.ResultBuffer '= New Autodesk.AutoCAD.DatabaseServices.ResultBuffer()
      Dim bRegApp As Boolean = DMAcadExt.AcadTransaction.RegistrateApp(DMAcadExt.TplnXDataParcel.XDataAppName)
      '?????	oXDataParcel = New DMAcadExt.TplnXDataParcel()
      Dim oValue As System.Object
      Dim iPgonID, iPgonIDNew As Integer
      Dim iRingType As Integer
      ...
      Try
         DMCommon.Debug.MsgBox("09_171", tShapeFOData.ShapeFileName, sClassName, tFilterList.List)
         bRes = GetParcelData(tShapeFOData.ShapeFileName, sClassName, tFilterList, dicParcelData, mdicGushFromParcelData, True)
      Catch oEx As Exception
      ...
      If dicParcelData IsNot Nothing Then
         If bRegApp Then
            iTestCounter = 0
            ...
            DMCommon.Debug.MsgBox("09_870", sClassName, CStr(lstPolygons.Count - 1), dicParcelData.Count, mdicGushFromParcelData.Count)
            ...
            For iIndex As Integer = 0 To lstPolygons.Count - 1
               ...
               iPgonID = iaPgonID(iIndex)
               iPgonIDNew = iPgonID - iFeatureIDMin + 1
               If dicParcelData.TryGetValue(iPgonIDNew, tParcelData) Then
   ...
   Public Sub LoadParcelData()
      ...
      If oFeatureReader IsNot Nothing Then
         ...
         While oFeatureReader.ReadNext()
            ...
            End Try
         End While
         DMCommon.Debug.MsgBox("12_620", "LoadParcelData", mtShapeFOData.Gush.Block, iPgonID)
      Else
...
2:    '@@@@@@@@@'oLinkDataRow
   ...
   Public Shared Sub InsertParcelData(sSHPFileName As String)
      ...
      reader.Close()
   End Sub
   Public Shared Function GetParcelData(sSHPFileName As String, sClassName As String, tFilterList As DMCommon.dmList, ByRef dicParcelData As Dictionary(Of Integer, ParcelData), ByRef dicGushFromParcelData As Dictionary(Of Integer, GushData), Optional bInclGeometria As Boolean = False) As Boolean
   ...
   Public Shared Function CopyParcelData(sSHPFileName As String, sClassName As String, tFilterList As DMCommon.dmList, sToSHPFileName As String) As Dictionary(Of Integer, ParcelData)
      ...
      If oFeatureReader IsNot Nothing Then
         ...
         End While
         DMCommon.Debug.MsgBox("08_561", sSHPFileName, sClassName, dicRes.Count, iPgonID)
         'DMCommon.Debug.MsgBox("01_424", oGeometry.Dimensionality, oGeometry.Text)
      Else
         System.Windows.Forms.MessageBox.Show("oFeatureReader Is NOTHING" & vbCrLf & sClassName, "04_409")
         ...
         While oFeatureReader.ReadNext()
            Try
               ...
               Else
                  iBlockAdd = 0
               End If
               dicRes.Add(iPgonID, New GushData(iPgonID, iBlock, iBlockAdd, iStatus, iIsAnality, 1952.0)) 'To Update !!!!!!
            Catch oEx As Exception
