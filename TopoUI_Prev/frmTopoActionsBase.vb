...
Public Class frmTopoActionsBase
	...
	Protected pbEventsEnabled As Boolean = False
	...
	Protected doSelectedReportItem As AcadReport.ReportItem
	...
	Private WithEvents cmdTest As TabButton
	Private WithEvents cmdTestA As TabButton


	Protected moaChecks(miToposUB) As TopoCheck
	Protected doProjectData As ProjectData	'''''''''''''''''''''''''''''
	Protected WithEvents txtStraightenTolerance As TextBox
	Protected WithEvents txtLegendPaintFactor As TextBox

	Protected WithEvents txtMinFDOSliverTolerance As TextBox
	Protected WithEvents txtMaxFDOSliverTolerance As TextBox
	...
	Protected moLabelBoldFont As System.Drawing.Font = New System.Drawing.Font("Tahoma", 9.0!, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, CType(177, Byte))
	...
	Private Sub zzMyInitializeComponent()
		...
		Me.cmdTest = New TabButton(moLabelFont, False)
		Me.cmdTestA = New TabButton(moLabelFont, False)
		...
		Me.tsiMapClearAll = New System.Windows.Forms.ToolStripMenuItem()
		Me.tsiMapClearLayer = New System.Windows.Forms.ToolStripMenuItem()
		Me.tsiParcelPgonsExp = New System.Windows.Forms.ToolStripMenuItem()
		...
		With Me.tstTopo
			...
			.Items.AddRange(New System.Windows.Forms.ToolStripItem() {Me.tsbSwitchCleanup, Me.tsbCreateTopo, Me.tsbCheckTopo, Me.tsbDeleteTopo, Me.tsbShowTopo, Me.tsbEraseTopoGeometria, Me.tsbCopyFromOverlay, Me.tsbToClosedPolygons, Me.tsbExportToShape, Me.tsbMapPlatf})
		...
		'
		With Me.cmdTest
			.Location = New System.Drawing.Point(516, 244)
			.Name = "cmdTest"
		...
		'
		With Me.cmdTestA
			.Location = New System.Drawing.Point(516, 220)
			.Name = "cmdTestA"
			.Size = New System.Drawing.Size(18, 24)
			.TabIndex = 15
			'    .Font = moLabelFont
		...
		'tsiMapClearAll
		'
		Me.tsiMapClearAll.CheckOnClick = False
		Me.tsiMapClearAll.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.Text
		Me.tsiMapClearAll.Name = "tsiMapClearAll"
		Me.tsiMapClearAll.Size = New System.Drawing.Size(134, 22)
		Me.tsiMapClearAll.Text = "ניקוי מפה"

		'
		'tsiMapClearLayer
		'
		Me.tsiMapClearLayer.CheckOnClick = False
		Me.tsiMapClearLayer.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.Text
		Me.tsiMapClearLayer.Name = "tsiMapClearLayer"
		Me.tsiMapClearLayer.Size = New System.Drawing.Size(134, 22)
		Me.tsiMapClearLayer.Text = "מחיקת השכבה"
		'
		'tsiParcelPgonsExp
		'
		Me.tsiParcelPgonsExp.CheckOnClick = False
		Me.tsiParcelPgonsExp.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.Text
		Me.tsiParcelPgonsExp.Name = "tsiParcelPgonsExp"
		Me.tsiParcelPgonsExp.Size = New System.Drawing.Size(134, 22)
		Me.tsiParcelPgonsExp.Text = "פוליגונים סגורים"
		zzSetGridColumns()
	...
	Protected Sub OnNew()
		...
		tGroupLocation(3).GroupYShiftIndex = -9
	...
	Private Sub zzSetCleanupErrPoints()
		'	System.Windows.Forms.MessageBox.Show(CStr(miCurrentAction) & ":" & moCurrentView.Count, "04_420")
	...
	Public Sub RefreshTopo()
		zzResetChecks()
	End Sub
	Protected Sub InsertReport(ByVal oDataView As DataView, ByVal iaDataColumns() As Integer, ByVal oaTotals() As System.Object, ByVal oaOptionValues() As System.Object, Optional ByVal iaEmptyColumns() As Integer = Nothing)
	...
	End Sub
	Protected Sub ResetCheck(ByVal tTopoDefID As DMAcadExt.TopoDefID)
		Dim iTopoIndex As Integer = TopoDefs.GetTopoIndex(tTopoDefID)

		If TopoManager.TopoDefs.moaTopoDefs(iTopoIndex) IsNot Nothing Then
			moaChecks(iTopoIndex).Checked = TopoManager.TopoCreator.TopologyExists(TopoManager.TopoDefs.moaTopoDefs(iTopoIndex))
		End If
		...
		If oTopoDef IsNot Nothing Then
			...
			Else
				If Information.IsNumeric(Me.txtTolerance.Text) Then
					...
					Try
						TopoCreator.CreateTopology(DMAcadExt.DMApp.AppID, oTopoDef, Me.chkHighlightSliver.Checked, dTolerance, False)
					Catch oEx As Exception
	...
	Private Sub chkTopoLayersOn_AutoSizeChanged(ByVal oSender As System.Object, e As System.EventArgs) Handles chkTopoLayersOn.AutoSizeChanged
         ...
         If bCurrentLayerOK Then
            ...
            If oTopoModel IsNot Nothing Then
               ...
               For Each oPolygon As Autodesk.Gis.Map.Topology.Polygon In colPolygons
                  ...
                  oColorPgon.CreateClosedPolygon(False)
   ...
   Private Sub zzCreateFDOLayer()
      ...
      If oTopoDef.ID.TopoIsMerge Then
         Dim iResTopDefID As DMAcadExt.TopoDefID = oTopoDef.ID
         ...
         Dim oSrcTopDef As DMAcadExt.TopoDef = TopoDefs.Item(stSrcTopDefID)
         Dim oOvlTopDef As DMAcadExt.TopoDef = TopoDefs.Item(stOvlTopDefID)
         'System.Windows.Forms.MessageBox.Show(oSrcTopDef.Name & vbCrLf & oOvlTopDef.Name, "01_805")
         Dim oSrcAddTopDef As DMAcadExt.TopoDef = oSrcTopDef.AdditionalTopoDef
         Dim oOvlAddTopDef As DMAcadExt.TopoDef = oOvlTopDef.AdditionalTopoDef
         'System.Windows.Forms.MessageBox.Show(oSrcAddTopDef.Name, "01_806")

         '	If TopoCreator.TopologyExists(oSrcAddTopDef) Then
         oSrcTopDef = oSrcAddTopDef
         'End If
         If TopoCreator.TopologyExists(oOvlAddTopDef) Then
            oOvlTopDef = oOvlAddTopDef
         End If

         Dim oFDO_Manager As FDO_Manager = New FDO_Manager()
         Dim bAllSlivers As Boolean = AllSlivers
         ...
         Else
            ...
            dMaxFDOSliverTolerance = MaxFDOSliverTolerance
         End If
         Dim sSHPFileName As String = oFDO_Manager.Union(oSrcTopDef, oOvlTopDef, oTopoDef, dMinFDOSliverTolerance, dMaxFDOSliverTolerance)

         '	System.Windows.Forms.MessageBox.Show(oMapImpExpEx.Message & vbCrLf & oMapImpExpEx.ErrorCode, "05_280")
         If sSHPFileName IsNot Nothing Then
            ...
            '	System.Windows.Forms.MessageBox.Show(sSHPFileName, "05_217")
            If TopoManager.TPlanGraph.TplnProject.ParcelGeoMethod = TopoManager.TPlanGraph.enGeoMethod.Topologia Then
               oFDO_Manager.DBF2XDataTopo(sSHPFileName, TopoManager.TPlanGraph.TplnProject.XDataAppName, False, False)
            End If
            If TopoManager.TPlanGraph.TplnProject.ParcelGeoMethod = TopoManager.TPlanGraph.enGeoMethod.ClosedPolygons Then
               oFDO_Manager.DBF2XDataCP(sSHPFileName, TopoManager.TPlanGraph.TplnProject.XDataAppName, False)
            End If
         End If
         '	zzOD2XData()
      ElseIf TopoCreator.TopologyExists(oTopoDef) Then
         If FDO_Manager.LayerExists(oTopoDef.FDOLayerName) Then

            System.Windows.Forms.MessageBox.Show("השכבה כבר קיימת" & vbCrLf & oTopoDef.Name & vbCrLf & oTopoDef.FDOConnectionName)
         Else

            If oTopoDef.ID.ID = 65 Then
            ...
            Dim sShapeFileName As String = System.Windows.Forms.Application.LocalUserAppDataPath & "\" & oTopoDef.Name & "\" & oTopoDef.Name & ".shp"
            Dim oDirectoryInfo As IO.DirectoryInfo = New IO.DirectoryInfo(System.Windows.Forms.Application.LocalUserAppDataPath & "\" & oTopoDef.Name)
            ...
            '''''''oShapeExpImp.LoadFomat(sFile)
            oShapeExpImp.FromTopology(sTopoName)
            oShapeExpImp.AddTopoData("ID", "AREA")
            '	oShapeExpImp.AddLayerFilter("0")
            oShapeExpImp.Exec()
            If False Then
               Dim colExpressionTarget As Autodesk.Gis.Map.ImportExport.ExpressionTargetCollection

               System.Windows.Forms.MessageBox.Show(sShapeFileName & vbCrLf & sTopoName, "01_800")
               Try
               ...
               Catch oMapImpExpEx As Autodesk.Gis.Map.MapImportExportException
                  ...
                  End If
                  System.Windows.Forms.MessageBox.Show(sExpMsg, "01_809a")
               End Try
               ...
               Catch oMapImpExpEx As Autodesk.Gis.Map.MapImportExportException
                  System.Windows.Forms.MessageBox.Show(oMapImpExpEx.Message & vbCrLf & oMapImpExpEx.ErrorCode & vbCrLf & sShapeFileName, "01_811")
               End Try
               Try
                  oExporter.SetExportFromPolygonTopology(True, sTopoName)
               Catch oMapImpExpEx As Autodesk.Gis.Map.MapImportExportException
               ...
               oExporter.LayerLevelMapping(True, oLayers)

               'oMyExporter.LoadExportFormat()	'FileOneEntityType


               '	System.Windows.Forms.MessageBox.Show(oLayers.Count & vbCrLf & oMapImpExpEx.ErrorCode, "01_823")
               Try
                  tRes = oExporter.Export()
               Catch oMapImpExpEx As Autodesk.Gis.Map.MapImportExportException
                  System.Windows.Forms.MessageBox.Show(oMapImpExpEx.Message & vbCrLf & oMapImpExpEx.ErrorCode, "01_823")
               End Try
            ...
            Try
               moFDO_Manager.ConnectToShape(oTopoDef.FDOConnectionName, sShapeFileName)
               moFDO_Manager.AddLayerToMap(oTopoDef.FDOConnectionName)
      ...
      DMAcadExt.AcadTransaction.Terminate()
   End Sub
   Public Shared Property MinFDOSliverTolerance() As Double

      Get
         Const sSettingName As String = "MinFDOSliverTolerance"
      ...
      End Set
   End Property
   Public Shared Property MaxFDOSliverTolerance() As Double

      Get
         Const sSettingName As String = "MaxFDOSliverTolerance"
         Dim oDynamic As System.Object = My.Settings.Item(sSettingName)
         If oDynamic IsNot Nothing Then
            Return DirectCast(oDynamic, Double)
         Else
      ...
      End Set
   End Property
   Public Shared Property AllSlivers() As Boolean

      Get
         Const sSettingName As String = "AllSlivers"
         Dim oDynamic As System.Object = My.Settings.Item(sSettingName)
         If oDynamic IsNot Nothing Then
            Return DirectCast(oDynamic, Boolean)
         Else
            Return False
         End If
      End Get
      Set(bValue As Boolean)
         Const sSettingName As String = "AllSlivers"
         My.Settings.Item(sSettingName) = bValue
      End Set
   End Property


   Private Shared Sub zzOD2XData()
      ...
      Dim lstMPolylgons As System.Collections.Generic.IList(Of Autodesk.AutoCAD.DatabaseServices.MPolygon) = Nothing
      '	System.Windows.Forms.MessageBox.Show("05_235")
      ...
      If oLinkTable.Exists Then
         ...
         '	lstMPolylgons = DMAcadExt.AcadTransaction.GetMPolygons(sPolylineLayer, Autodesk.AutoCAD.DatabaseServices.OpenMode.ForWrite)
         For Each oPolygon As Autodesk.AutoCAD.DatabaseServices.MPolygon In lstMPolylgons
            ...
            Catch oMapEx As Autodesk.Gis.Map.MapException
               ...
               System.Windows.Forms.MessageBox.Show(oMapEx.Message & vbCrLf & sTableName, "04_207f")
            End Try
            If oODRec IsNot Nothing Then
               ...
               Try
                  ...
                  If iTestCounter < 6 Then
                     oEditor.WriteMessage("27-- " & ":" & CStr(iFeatureID) & ":" & CStr(iParcelID) & ":" & CStr(iLotID) & vbCrLf)
                  End If

               Catch oEx As Exception

                  System.Windows.Forms.MessageBox.Show(oEx.Message, "04_209")
               End Try
               Dim sAppName As String = TopoManager.TPlanGraph.TplnProject.XDataAppName
               ...
               If bRes Then
                  ...
                  oResBuffer.Add(oVal)
                  oVal = New Autodesk.AutoCAD.DatabaseServices.TypedValue(1071, iFeatureID)
                  oResBuffer.Add(oVal)
                  oVal = New Autodesk.AutoCAD.DatabaseServices.TypedValue(1071, iParcelID)
                  oResBuffer.Add(oVal)
                  oVal = New Autodesk.AutoCAD.DatabaseServices.TypedValue(1071, iLotID)
                  oResBuffer.Add(oVal)
   ...
   Private Sub zzExportToShape()
      ...
      If TopoCreator.TopologyExists(oTopoDef) Then
         ...
         Try
            ...
            System.Windows.Forms.MessageBox.Show(CStr(oRes.EntitiesExported), "01_813")
   ...
   Private Sub cmdExit_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdExit.Click
      ...
      DMAcadExt.AppMessages.ClearAll()
      DMAcadExt.AppMessages.Terminate()
   ...
   End Sub
	...
	Private Sub cmdTest_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdTest.Click
		Dim oTopoDef As DMAcadExt.TopoDef = zzLoadTopoDef()
		System.Windows.Forms.MessageBox.Show(oTopoDef.Name, "02_700")
		FDO_Manager.NewTestQ(oTopoDef.Name)
	End Sub
	Private Sub cmdTestA_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdTestA.Click
		'	Dim oFDO_Manager As FDO_Manager = New FDO_Manager()
		Try

			'	zzDeleteFDOLayer()
			'	moFDO_Manager.RemoveLayer()
			'	oFDO_Manager.TestA()

			''''''''''''''oFDO_Manager.TestRemove()

		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActionsBase - cmdTestA_Click")
		End Try
	...
	Protected MustOverride Sub OnUpdateProjectData()
	...
	Private Sub zzSetCommonButtons(ByVal iTabIndex As Integer)
		Select Case iTabIndex
			Case 0
				...
				Me.tbpTopo.Controls.Add(Me.cmdTest)
				Me.tbpTopo.Controls.Add(Me.cmdTestA)
	...
	Protected Overridable Sub zzInitTabMessages()
		...
		With txtMessageText
			...
			.RightToLeft = Windows.Forms.RightToLeft.Yes
		End With
		Me.tbpMessages.Controls.Add(Me.txtMessageText)
		...
		Me.cmdTableMsg = New TabButton(moLabelFont, False)
		'
		'cmdTableMsg
		'
		With Me.cmdTableMsg
			...
			.Text = "Table"
		End With
		Me.tbpMessages.Controls.Add(Me.cmdTableMsg)
	...
	Private Sub zzDeleteTopo(ByVal bDeleteEntities As Boolean)
		...
		If oTopoDef IsNot Nothing Then
			...
			iTopoTypeIndex = moCurrentLabel.Index
			Me.moaChecks(iTopoTypeIndex).Checked = TopoCreator.TopologyExists(oTopoDef)
			If oTopoDef.ID.TopoIsBase Then
				Dim tDisTopoDefID As DMAcadExt.TopoDefID = oTopoDef.ID.DissolveID
				Dim tDisTopoDef As DMAcadExt.TopoDef = New DMAcadExt.TopoDef(DMAcadExt.DMApp.AppID, tDisTopoDefID)
				TopoCreator.DeleteTopology(oTopoDef, bDeleteEntities)
			End If
		End If
	End Sub
	Private Sub zzCheckTopo()
		...
		If oTopoDef IsNot Nothing AndAlso TopoCreator.TopologyExists(oTopoDef) Then
			...
			If oTopoModel IsNot Nothing Then
				Try
					...
					For Each oPolygon As Autodesk.Gis.Map.Topology.Polygon In colPolygons
						...
						oPolygon = Nothing
					Next
					Dim iCount As Integer = colPolygons.Count
					If iCount = 0 Then
						sMsg &= "empty"
					ElseIf oTopoModel.IsComplete Then
						sMsg &= "correct and complete"

						If iCount = 1 Then
							sMsg &= "," & vbCrLf & "contains one polygon"
						ElseIf iCount > 1 Then
							sMsg &= "," & vbCrLf & "contains " & CStr(iCount) & " polygons"
						End If
			...
			Else
				...
				iMessageBoxIcon = MessageBoxIcon.Error
			End If
			MessageBox.Show(sMsg, Me.Text, MessageBoxButtons.OK, iMessageBoxIcon, MessageBoxDefaultButton.Button1)
			DMAcadExt.AcadDocument.Unlock()
		End If
	End Sub
	...
	Private Sub zzEraseTopoGeometria()
		...
		Dim sCentroidLayer As String = oTopoDef.CreateTopologyLayer
		If oTopoDef IsNot Nothing Then
			DMAcadExt.AcadDocument.DocLock(Autodesk.AutoCAD.ApplicationServices.DocumentLockMode.ExclusiveWrite, True)
			DMAcadExt.AcadTransaction.Start()
			DMAcadExt.AcadTransaction.OpenModelSpace(Autodesk.AutoCAD.DatabaseServices.OpenMode.ForWrite)
			If bTopologyExists Then
				...
				Else
					TopoCreator.DeleteTopology(sTopoName, True, False)
				End If
			...
			End If
			If sCentroidLayer.Length <> 0 Then
				DMAcadExt.AcadTransaction.ClearLayerByClassName(sCentroidLayer, DMAcadExt.AcadConst.AcadPointName)
			End If
		...
		End If
	End Sub
	Private Sub zzPrepare()	'Straighten
		Dim oTopoDef As DMAcadExt.TopoDef = zzLoadTopoDef()
		...
		ElseIf oTopoDef.ID.TopoIsAdditional Then
			...
			If dStraightenTolerance > 0.00001 Then
				DMAcadExt.dmLineCleanup.Straighten(oTopoDef, dStraightenTolerance)
			End If
			...
			If bCurrentLayerOK Then
				...
				If iAcadActionUB >= 0 Then
					...
					Try
						...
						If oTopoDef IsNot Nothing Then
							Try
								'	MessageBox.Show(oTopoDef.IncludeLayers & ":" & oTopoDef.LinkLayers, "26_315")
								iaErrors = TopoManager.TopoCreator.CleanupByTopoDef(oaActionVar, oTopoDef, bFix, moaErrorPoints)
							Catch oEx As Exception
								MessageBox.Show(oEx.Message & vbCrLf & oEx.GetType().ToString(), "26_998")
	...
	Private Sub zzLoadData(ByVal iTopoTypeIndex As Integer)
		...
		If moDataTable(iTopoTypeIndex) Is Nothing Then
			...
			moOleDbDataAdapter(iTopoTypeIndex) = TPlServerDB.ServerDB.CurrentServerDB.GetDataAdapter(sSelectComText, CommandType.Text, True)
	...
	End Sub
	Private Sub tsbViews_DropDownItemClicked(ByVal oSender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tsbMapPlatf.DropDownItemClicked
		Dim oToolStripItem As System.Windows.Forms.ToolStripItem = e.ClickedItem
		Select Case oToolStripItem.Name
			Case Me.tsiMapClearAll.Name
				zzClearAllMap()
			Case Me.tsiMapClearLayer.Name
				zzClearMapLayer()
			Case tsiParcelPgonsExp.Name
				zzParcelPgonsExp()
	...
	Private Sub zzClearMapLayer()
		Dim oTopoDef As DMAcadExt.TopoDef = zzLoadTopoDef()
		DMAcadExt.AcadDocument.DocLock(Autodesk.AutoCAD.ApplicationServices.DocumentLockMode.ExclusiveWrite, True)

		'	System.Windows.Forms.MessageBox.Show(oTopoDef.FDOConnectionName & vbCrLf & oTopoDef.FDOLayerName, "zzClearMapLayer - 01_821")
      FDO_Manager.RemoveConnectionB(oTopoDef.FDOConnectionName)
		FDO_Manager.RemoveResource(oTopoDef.FDOConnectionName, oTopoDef.FDOLayerName)
		DMAcadExt.AcadDocument.Unlock()


	End Sub
	Private Sub zzClearAllMap()
		DMAcadExt.AcadDocument.DocLock(Autodesk.AutoCAD.ApplicationServices.DocumentLockMode.ExclusiveWrite, True)

		'	moFDO_Manager = New FDO_Manager()
		'	System.Windows.Forms.MessageBox.Show(oTopoDef.FDOConnectionName & vbCrLf & oTopoDef.FDOLayerName, "zzClearMapLayer - 01_821")
		'		FDO_Manager.RemoveConnectionB(oTopoDef.FDOConnectionName, oTopoDef.FDOLayerName)
		'		FDO_Manager.RemoveResource(oTopoDef.FDOLayerName)

		'	FDO_Manager.RemoveLayer(oTopoDef.FDOLayerName)
		'	moFDO_Manager.RemoveConnection(sConnectionName)
		'	FDO_Manager.DeleteRecource(sConnectionName)
		'	FDO_Manager.ClearAllLayers()
	...
	Private Sub zzParcelPgonsExp()
		MessageBox.Show("", "01_500")
		Dim oTopoDef As DMAcadExt.TopoDef = zzLoadTopoDef()
		...
		Else
			...
			Else
				...
				System.Windows.Forms.MessageBox.Show(sShapeFileName & vbCrLf & sTopoName, "01_801")
				Dim oShapeExpImp As ShapeExpImp = New ShapeExpImp(enExpImp.Export, sShapeFileName)
