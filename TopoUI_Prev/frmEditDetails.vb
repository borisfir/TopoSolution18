...
Public Class frmEditDetails
	...
	Private miDeatailSource As Integer
	...
	Private Sub zzSetDetails()
		...
		moDataAdapter = TPlServerDB.ServerDB.CurrentProjectDB.GetDataAdapter(sComText, CommandType.Text, True, , True, True)
		...
		If iRowsCount = 0 Then
			miDeatailSource = 0
		Else
			miDeatailSource = DirectCast(moDetails.Rows.Item(iRowsCount - 1).Item(1), Integer)

		End If

		If mtID_Control.BelongRange(miDeatailSource) Then
			mtID_Control.SetID(miDeatailSource)
		Else
			mtID_Control.Reset()
	...
	End Sub
	Private Function zzAddPrjMapTheme(iNewDeatailNo As Integer, Optional iDeatailSource As Integer = -1) As Integer
		...
		If iDeatailSource = -1 Then
			iDeatailSource = miDeatailSource
		End If
		oaParams(0) = TPlServerDB.ServerDB.CurrentProjectDB.GetParameter("@prProjectCode", DbType.Int32, miProjectCode)
		oaParams(1) = TPlServerDB.ServerDB.CurrentProjectDB.GetParameter("@prDetailSource", DbType.Int32, iDeatailSource)
		oaParams(2) = TPlServerDB.ServerDB.CurrentProjectDB.GetParameter("@prNewDetail", DbType.Int32, iNewDeatailNo)

		iResRows = TPlServerDB.ServerDB.CurrentProjectDB.RunCommand(sSPName, CommandType.StoredProcedure, oaParams, oErrOut)
	...
	Private Sub moDetails_TableNewRow(oSender As System.Object, e As System.Data.DataTableNewRowEventArgs) Handles moDetails.TableNewRow
		...
		'	MessageBox.Show(Date.Now.ToString(), "01_291 Table!!!")
	End Sub
