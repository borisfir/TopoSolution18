Option Explicit On
Option Strict On
Public Class frmTopoActionsOld
	...
	Private Shared miResourceTheme As TPlServerDB.enResourceTheme = TPlServerDB.enResourceTheme.AcFrmActionsBase
	Private miToposUB As Integer = TopoDefs.giToposUB
...
, TPlanGraph.enDataOptions.Default}
	...
	Private Sub frmTopoActions_Disposed(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles Me.Disposed
		...
		Try
			...
			Dim baCalculateSetting() As Boolean = {Me.chkParcel.Checked _
			 ...
			 , Me.chkMerge.Checked, Me.chkUnion.Checked}
			sTest = "b"
	...
	Private Sub zzMyInitializeComponent()
		...
		'
		With Me.chkMerge
			.Location = New System.Drawing.Point(8, 92)
			.Name = "chkMerge"
			.Size = New System.Drawing.Size(120, 24)
			.TabIndex = 22
			.Font = moLabelFont
			...
			Try
				For iIndex As Integer = 0 To iActionUB
					If Not Me.dgvMain.Rows.Item(iIndex).IsNewRow Then
						...
						dTolerance = DMCommon.Functions.CDblN(Me.dgvMain.Rows.Item(iIndex).Cells(msToleranceFldName).Value)
				...
				If iActionUB >= 0 Then
					...
					If oTopoDef IsNot Nothing Then
						...
						Catch oEx As Exception
							...
							Common.GetMapTopoEx(oEx, "C919a_old")
							Exit Sub
						End Try
	...
	Private Sub zzEraseTopoGeometria()
		...
		If oTopoDef IsNot Nothing Then
			...
			DMAcadExt.AcadTransaction.ClearLayerByClassName(oTopoDef.LinkLayer, "AcDbPoint", False)
		...
		If oTopoDef IsNot Nothing Then
			...
			Else
				If Information.IsNumeric(Me.txtTolerance.Text) Then
					...
					TopoCreator.CreateTopology(1, oTopoDef, Me.chkIgnoreIncompleteArea.Checked, Me.chkHighlightSliver.Checked, dTolerance)
				Else
					Beep()
				End If
			End If
	...
	Private Sub zzTopoLayersIsOnAAA_BBB()
		...
		Me.chkTopoLayersOn.CheckState = DMAcadExt.AcadTransaction.LayersIsOn(1, saTopoLayers, True)
		...
		If Me.lstReports.SelectedItem IsNot Nothing Then
			...
			If oDataView IsNot Nothing Then
				...
				If oRepApp.AcadModel Then
					...
					bCurrentLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(String.Empty, 1, DMAcadExt.enLayerFunction.Report, True, True, True)
	...
	Private Class CheckButton
		...
		Public Sub SetImage()
			If MyBase.CheckState = System.Windows.Forms.CheckState.Checked Then
				MyBase.Image = Global.TopoManager.My.Resources.Resources.LayerOn
	...
	Private Sub tabMain_SelectedIndexChanged(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles tabMain.SelectedIndexChanged
		...
		Select Case Me.tabMain.SelectedIndex
			Case 1
				If Me.moaChecks(0).CheckState = CheckState.Checked Then
					Me.chkParcel.Enabled = True
				Else
					Me.chkParcel.Checked = False
					Me.chkParcel.Enabled = False
				End If
				If Me.moaChecks(1).CheckState = CheckState.Checked Then
					Me.chkApproved.Enabled = True
				Else
					Me.chkApproved.Checked = False
					Me.chkApproved.Enabled = False
				End If
				If Me.moaChecks(2).CheckState = CheckState.Checked Then
					Me.chkProposed.Enabled = True
				Else
					Me.chkProposed.Checked = False
					Me.chkProposed.Enabled = False
				End If
	...
	Private Function zzCheckCalculateOptionsOld() As Boolean
		Dim bLotNotExists As Boolean = (Not Me.chkApproved.Checked) And (Not Me.chkProposed.Checked)
		Dim bParcelNotExists As Boolean = Not Me.chkParcel.Checked
		If (bLotNotExists Or bParcelNotExists) And Me.chkUnion.Checked Then
			Me.chkUnion.Checked = False
			Beep()
			Return False
		Else
			Return True
	...
	Private Sub cmdCalculate_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdCalculate.Click
		...
		TPlanGraph.TplnProject.Calculate(Me.chkApproved.Checked, Me.chkProposed.Checked, Me.chkParcel.Checked, Me.chkMerge.Checked, Me.chkUnion.Checked)
	...
	Private Sub cmbBlockFormat_SelectedIndexChanged(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmbBlockFormat.SelectedIndexChanged
		DMAcadExt.AcadBlockDef.Format = Me.cmbTopoFormat.SelectedIndex + 1
	End Sub
