...
Public Class TplnPolygonSet
   ...
   End Sub
   Public Shared Function PolygonRelation(oPolyline As Polyline, oPolylineA As Polyline) As PgonRelation
   ...
   End Sub
   Public Sub SetDataInsertBlock(hsGush As HashSet(Of Integer), ByRef tShapeFOData As FDO_Manager.ShapeFOData)
      If miMapTheme = DMAcadExt.enMapTheme.Parcels Then

         SetDataInsertBlock_Parcel(hsGush, tShapeFOData)
      ElseIf miMapTheme = DMAcadExt.enMapTheme.Blocks Then
         TopoManager.TPlanGraph.TplnBlock.Initialize("1601")
         SetDataInsertBlock_Gush(tShapeFOData, "1601", "1601")
            ...
            For Each oRow As DataRowView In oPointsOutside
               ...
               With oRow
                  If miMapTheme = DMAcadExt.enMapTheme.Parcels Then
                     ...
                     .Item(msLegalAreaNameFieldName) = oParcel.LegalArea
                  ElseIf miMapTheme = DMAcadExt.enMapTheme.LotApproved Then
   ...
   Public Sub SetFeatureIDMin()
      ...
      End If
   End Sub
   Public Sub SetDataInsertBlock_Gush(ByRef tShapeFOData As FDO_Manager.ShapeFOData, sBlockName As String, sLayerName As String)
      sLayerName = "1601"
      sBlockName = "1601"
      Dim oEditor As Autodesk.AutoCAD.EditorInput.Editor = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor
      ...
      Try
         dicRes = GetGushData(tShapeFOData.ShapeFileName, sClassName)
      ...
      If dicRes IsNot Nothing Then
         ...
         End If
      Else
         System.Windows.Forms.MessageBox.Show("OverlayData was not found" & vbCrLf & tShapeFOData.ShapeFileName, "04_273")
            ...
            If oResBuffer IsNot Nothing Then
               ...
               If mdicParcelData.TryGetValue(iPgonIDNew, tParcelData) Then
                  If iIndex = 0 Then
                     ...
                     mtShapeFOData.Gush = New GushData(0, tParcelData.Block, tParcelData.BlockAdd, tParcelData.Status, 0)
                  End If
                  miTopoIDCounter += 1
                  oXDataParcel.ID = miTopoIDCounter
                  oXDataParcel.DataID = iPgonIDNew
   ...
   End Sub
   Public Sub SetDataInsertBlock_Parcel_250115(setGush As HashSet(Of Integer), ByRef tShapeFOData As FDO_Manager.ShapeFOData)
      Dim oEditor As Autodesk.AutoCAD.EditorInput.Editor = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor
      Dim oMapApplication As Autodesk.Gis.Map.MapApplication = Autodesk.Gis.Map.HostMapApplicationServices.Application
      Dim oMapUtility As Autodesk.Gis.Map.MapUtility = oMapApplication.ActiveProject.MapUtility
      Dim colCentroids As Autodesk.AutoCAD.DatabaseServices.ObjectIdCollection = New Autodesk.AutoCAD.DatabaseServices.ObjectIdCollection()
      ...
      Dim lstPolygons As System.Collections.Generic.IList(Of Autodesk.AutoCAD.DatabaseServices.Polyline)
      Dim sClassName As String = tShapeFOData.FeatureClass
      Dim dicRes As Dictionary(Of Integer, ParcelData)
      Dim tParcelData As ParcelData = Nothing
      Dim oResBuffer As Autodesk.AutoCAD.DatabaseServices.ResultBuffer '= New Autodesk.AutoCAD.DatabaseServices.ResultBuffer()
      Dim bRes As Boolean = DMAcadExt.AcadTransaction.RegistrateApp("CPTopo_Parcels")
      '?????	oXDataParcel = New DMAcadExt.TplnXDataParcel()
      ...
      Dim dicMPgons As Dictionary(Of Integer, Autodesk.AutoCAD.DatabaseServices.MPolygon) = New Dictionary(Of Integer, Autodesk.AutoCAD.DatabaseServices.MPolygon)()
      Try
         dicRes = GetParcelData(tShapeFOData.ShapeFileName, sClassName)
      Catch oEx As Exception
         System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & tShapeFOData.ShapeFileName & vbCrLf & sClassName, "04_999")
         dicRes = Nothing
      End Try
      '	Dim o As Autodesk.AutoCAD.DatabaseServices.ImpEntity
      Dim oPolygon As Autodesk.AutoCAD.DatabaseServices.Polyline
      ...
      If dicRes IsNot Nothing Then
         If bRes Then
            ...
            System.Windows.Forms.MessageBox.Show(CStr(lstPolygons.Count - 1) & vbCrLf & CStr(dicRes.Count), "04_247s")
            Dim iaPgonID(lstPolygons.Count - 1) As Integer
            Dim iaCentroidIndecis(lstPolygons.Count - 1) As Integer
            Dim iCentroidIndex As Integer = 0
               ...
               If oResBuffer IsNot Nothing Then
                  ...
                  If dicRes.TryGetValue(iPgonIDNew, tParcelData) Then
                     If iIndex = 0 Then
                        ...
                        '	tShapeFOData.Block = tParcelData.Block
                        '	tShapeFOData.BlockAdd = tParcelData.BlockAdd
                        tShapeFOData.Gush = New GushData(0, tParcelData.Block, tParcelData.BlockAdd, tParcelData.Status, 0)
                     End If
                     miTopoIDCounter += 1
                     oXDataParcel.ID = miTopoIDCounter
                     oXDataParcel.DataID = iPgonIDNew
                     ...
                     If iRingType = 1 Then
                        ...
                        End If
                     Else
                        System.Windows.Forms.MessageBox.Show("#" & CStr(iPgonIDNew), "01_892s")
                        oPolygon.Erase()
                     End If
                  Else
      ...
      End If
   End Sub
   Public Sub SetDataInsertBlock_Parcel(setGush As HashSet(Of Integer), ByRef tShapeFOData As FDO_Manager.ShapeFOData)
      ...
      If dicRes IsNot Nothing Then
         If bRes Then
            ...
            System.Windows.Forms.MessageBox.Show(CStr(lstPolygons.Count - 1) & vbCrLf & CStr(dicRes.Count), "04_247x")
            Dim iaPgonID(lstPolygons.Count - 1) As Integer
   ...
   Public Sub LoadParcelData()
      ...
      Dim oFeatureReader As OSGeo.FDO.Commands.Feature.IFeatureReader = zzGetReader(mtShapeFOData.ShapeFileName, mtShapeFOData.FeatureClass)
   ...
   Private Sub zzLoadImport(colPolylineIDs As ObjectIdCollection)
      ...
      Else
         ...
         For Each tPolylineObjID As ObjectId In colPolylineIDs
            ...
            If oResBuffer IsNot Nothing Then
               ...
               If taTypedValues.GetUpperBound(0) >= 3 Then
                  ...
                  Catch oEx As Exception
                     System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & iNLoopTest & vbCrLf & CStr(iPgonImpID) & ":" & CStr(mdicImpMPgons.Count), "04_178")
         ...
         Next
         '		System.Windows.Forms.MessageBox.Show(CStr(miaFeaturesID.Count) & ":" & CStr(mcolOuterPolylineIDs.Count), "04_199")
      End If
   End Sub
   ...
   Private Sub zzFillLinkTable()
      ...
      For Each tAcObjID As ObjectId In mcolPolylineIDs
         ...
         If miSource = enPolygonSetSource.Shapes OrElse miSource = enPolygonSetSource.NotDefined Then
            oXDataParcel = New DMAcadExt.TplnXDataParcel(oPolyline.GetXDataForApplication("CPTopo_Parcels"), False)
               ...
               Else ' miSource = enPolygonSetSource.Polylines  
                  ...
                  If colCentroidsObjID.Count = 1 Then
                     ...
                     If miMapTheme = DMAcadExt.enMapTheme.Parcels Then
                        ...
                        oParcel = New TopoManager.TPlanGraph.TplnParcel(oResMPolygon, 0, tCentroidAcObjID)
                        oXDataParcel.Block = oParcel.BlockNo
                        oXDataParcel.BlockAdd = 0
                        If oParcel.Name IsNot Nothing Then
                     ...
                     ElseIf miMapTheme = DMAcadExt.enMapTheme.LotApproved Then
                        Dim oLot As TopoManager.TPlanGraph.TplnLot
                        oLot = New TopoManager.TPlanGraph.TplnLot(miTopoPurpose, oResMPolygon, 0, tCentroidAcObjID)
                        If oLot.Name IsNot Nothing Then
...
2:    '@@@@@@@@@'oLinkDataRow
   ...
   Public Function ExportByGroup() As FDO_Manager.ShapeFOData()
      ...
      For iRowIndex As Integer = 0 To oLinkViewByGroup.Count - 1
         ...
         If iGroupID <> iCurrentGroupID Then
            ...
            zzExportGroup(colLinkObjIDs, sShapeFileName)
            iShapeFileNameUB += 1
            ReDim Preserve taShapeFileNames(iShapeFileNameUB)
            oFileInfo = New IO.FileInfo(sShapeFileName)
            taShapeFileNames(iShapeFileNameUB) = New FDO_Manager.ShapeFOData(oFileInfo.Directory.FullName, sFeatureClass)
            taShapeFileNames(iShapeFileNameUB).Gush = New GushData(0, iCurrentGroupID, 0, 0, 0)
   ...
   Private Sub zzExportGroup(colLinkObjIDs As ObjectIdCollection, sShapeFileName As String)
      ...
      Catch oMapImpExpEx As Autodesk.Gis.Map.MapImportExportException
         'Dim o As Object = oMapImpExpEx.GetObjectData(
   ...
   Private Function zzGetCentroidsInMPgon(oMPolygon As MPolygon, sExteriorLineHandle As String) As ObjectIdCollection
         ...
         For iIndex As Integer = 0 To oCentroidDataView.Count - 1
            ...
            ElseIf (iaLoopInd.Count = 1) Then '(iaLoopInd.Count = 1 AndAlso iaLoopInd.Item(0) <> 0) OrElse iaLoopInd.Count > 1 Then
               ...
               If iaLoopInd.Item(0) = iExteriorIndex Then
                  ...
                  colRes.Add(tAcObjID)
               Else
                  sMsg &= ";*" & CStr(iaLoopInd.Item(0)) & "C" & CStr(tPoint.X) & "," & CStr(tPoint.Y)
               End If
      ...
      If colRes.Count = 0 Then
         DMAcadExt.AcadDocument.WriteMessage("Points in Ext = " & CStr(oCentroidDataView.Count) & "|" & sMsg)
      End If
      Return colRes
   End Function
   ...
   End Function
   Private Shared Function zzGetReader(sSHPFileName As String, sClassName As String) As OSGeo.FDO.Commands.Feature.IFeatureReader
      ...
      Return oSelect.Execute()
   End Function
   Public Shared Function GetClipF_ID(sSHPFileName As String, sClassName As String) As Dictionary(Of Integer, ParcelData)
      Dim dicRes As Dictionary(Of Integer, ParcelData) = New Dictionary(Of Integer, ParcelData)()
      Dim oFeatureClass As Schema.ClassDefinition
      Dim oFeatureReader As OSGeo.FDO.Commands.Feature.IFeatureReader = zzGetReader(sSHPFileName, sClassName)
      If oFeatureReader IsNot Nothing Then
         Dim iGeo As Integer = 0
         Dim sText As String
         Dim iPgonID As Integer
      ...
      Return dicRes
   End Function
   Public Shared Function GetParcelData(sSHPFileName As String, sClassName As String) As Dictionary(Of Integer, ParcelData)
      Dim dicRes As Dictionary(Of Integer, ParcelData) = New Dictionary(Of Integer, ParcelData)()
      Dim oFeatureClass As Schema.ClassDefinition
      Dim oFeatureReader As OSGeo.FDO.Commands.Feature.IFeatureReader = zzGetReader(sSHPFileName, sClassName)
      If oFeatureReader IsNot Nothing Then
      ...
      Return dicRes
   End Function
   Public Shared Function GetGushData(sSHPFileName As String, sClassName As String) As Dictionary(Of Integer, GushData)
         ...
         While oFeatureReader.ReadNext()
            Try
               ...
               Else
                  iBlockAdd = 0
               End If
               dicRes.Add(iPgonID, New GushData(iPgonID, iBlock, iBlockAdd, iStatus, iIsAnality))
            Catch oEx As Exception
   ...
   Public Sub New(iMapTheme As DMAcadExt.enMapTheme, iTopoPurpose As DMAcadExt.enTopoPurpose, sName As String)
      ...
      '	System.Windows.Forms.MessageBox.Show(iMapTheme.ToString() & vbCrLf & iTopoPurpose.ToString & vbCrLf & sName & vbCrLf & miSource.ToString, "03_122")
      '	miSource = enPolygonSetSource.Polylines
   End Sub
   Public Sub New(ByVal iMapTheme As DMAcadExt.enMapTheme)
      ...
      '   mbParcel = bParcel
      miSource = enPolygonSetSource.Shapes
      '	System.Windows.Forms.MessageBox.Show(CStr(bParcel) & vbCrLf & miSource.ToString, "03_120")
   End Sub
   Public Sub New(ByVal tShapeFOData As FDO_Manager.ShapeFOData, ByVal bParcel As Boolean)
      ...
      zzCreateLinkTable()
      zzCreateCentroidTable()
      '''''''''''''''''''''	System.Windows.Forms.MessageBox.Show(CStr(bParcel) & vbCrLf & miSource.ToString, "03_121")
   End Sub
   ...
   Public Class FindCenter
      ...
      Public Function GetInnerPoint() As DMAcadExt.TPlnPoint
         ...
         DMAcadExt.AcadDocument.WriteMessage("CntrPoint:" & oCenterPoint.Coordinates)
      ...
      Private Function zzCheckPoint(oPoint As DMAcadExt.TPlnPoint) As Boolean
         Dim iaLoopInd As IntegerCollection
         iaLoopInd = moMPgon.IsPointInsideMPolygon(oPoint.GetPoint3d(), mdToler)
         If iaLoopInd IsNot Nothing AndAlso iaLoopInd.Count > 0 Then
...
Public Class PgonRelation
   ...
   Public Sub Calculate(bPrintMessages As Boolean)
      ...
      If moPolylineA.Closed AndAlso moPolylineB.Closed Then
         ...
         If bCrossed1 OrElse bCrossed2 OrElse Not mbIsBalanced Then
            'Err
         ...
         ElseIf bHasInterior Then
            ''
            moPolylineA.IntersectWith(moPolylineB, Intersect.OnBothOperands, colPoints, New IntPtr(0), New IntPtr(0))
            '   DMAcadExt.AcadDocument.WriteMessage("Int IntW points: " & CStr(colPoints.Count))
            If colPoints.Count = 0 Then
               'Island
               mbIsCorrect = True
         ...
         Else
            ...
            If mbIsTouch Then
               ...
               If colPoints.Count > 2 Then
                  'DMAcadExt.AcadDocument.WriteMessage("IncludesTouchingLoops. Intersec points: " & CStr(colPoints.Count))
                  'colPoints.Clear()
               End If
            Else
               'CLUM
   ...
   Private Function zzPolylineToArray(oPolyline As Polyline) As Point2d()
      ...
      DMAcadExt.AcadDocument.WriteMessage("Dist Min= " & CStr(dMin))
      Return iaResPoints
   End Function
