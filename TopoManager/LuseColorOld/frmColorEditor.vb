...
Public Class frmColorEditor
		...
		If miFormMode = enColorEditorMode.Landuse Then
			...
			If Me.cmbColorSchemes.Items.Count > 0 Then
				If miLanduseID = 0 Then
					'	Me.cmbColorSchemes.Text = Me.cmbColorSchemes.Items(0).ToString()
				Else
					'	Me.cmbColorSchemes.SelectedValue = miLanduseID
					'	MessageBox.Show(CStr(Me.cmbColorSchemes.Items.Count) & ":" & CStr(miLanduseID), "18_900")
				End If
	...
	End Sub
	Private Function zzGetFormIndex(ByVal iColorIndex As Integer) As Integer
	...
	Public Sub New(ByVal iMode As enColorEditorMode, Optional ByVal iColorSchemeID As Integer = 0)

		' This call is required by the Windows Form Designer.
		InitializeComponent()
		miFormMode = iMode
		If iMode = enColorEditorMode.Landuse Then
			miLanduseID = iColorSchemeID
		End If
	...
	Private Function zzGetNewColorSchemeID(ByVal bLanduse As Boolean) As Integer
		...
		If sMax.Length <> 0 Then
			sCom &= " AND (ID <= " & sMax & ")"
		End If
		Dim oRes As System.Object = TPlServerDB.ServerDB.GetDataScalar(sCom)
	...
	Private Sub zzUpdate()
		...
		'mtColorSchemeDB.ID <> 0 OrElse
		If (miFormMode = enColorEditorMode.Landuse AndAlso miLanduseID <> 0) OrElse miFormMode = enColorEditorMode.ColorScheme Then
	...
	Private Sub zzBuildColorScheme()
		...
		Try
			If Me.cmbZebraAngle.Text.Length <> 0 Then
		...
		Try
			Dim sHatchName As String = Me.cmbHatchName.Text
			Dim tHatch As DMAcadExt.DMHatch = New DMAcadExt.DMHatch(sHatchName)
			With tHatch
				.Scale = 1
				.BackColor = mtaDMColors(0)
				If (sHatchName IsNot Nothing) AndAlso sHatchName.Length <> 0 Then
			...
			DMAcadExt.AcadDocument.WriteMessage("HasHatch: " & CStr(mtColorScheme.HasHatch()) & ":" & CStr(mtColorScheme.Hatch.SolidOnly))
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & oEx.StackTrace, "frmEditColorScheme")
		End Try
	...
	Private Sub txtWidth_DoubleClick(ByVal oSender As Object, ByVal e As System.EventArgs)
		Dim oTextBox As TextBox
		Dim iColorIndex As Integer
		Dim tDMColor As DMAcadExt.DMColor
		Try
	...
	Private Sub pcbPicture_Paint(ByVal oSender As Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles pcbPicture.Paint
		zzPaint(e.Graphics)
	End Sub
