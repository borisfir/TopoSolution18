...
Public Class frmTplnView
   ...
   End Sub
	...
	Private Sub zzFilterBySelect()
		...
		If oCurrentCell IsNot Nothing Then
			...
			Select Case sFieldName
				Case TplnParcel.msBlockFieldName
					sFilterFieldName = TplnParcel.msBlockFieldName
				Case TplnParcel.msNameFieldName, TopoReader.msParcelTopoIDFldName
					sFilterFieldName = TopoReader.msParcelTopoIDFldName
				Case TplnParcel.msLotNameFieldName, TopoReader.msLotTopoIDFldName
					sFilterFieldName = TopoReader.msLotTopoIDFldName
				Case TplnParcel.msLanduseIDFieldName, TplnParcel.msLanduseNameFieldName
					sFilterFieldName = TplnParcel.msLanduseIDFieldName
			End Select
			If sFilterFieldName IsNot Nothing Then
	...
	Private Sub zzZoomBoard(ByVal iTopoID As Integer, ByVal iNeighborTopoID As Integer, ByVal bHighlight As Boolean)
		...
		If iTopoID <> 0 Then
			...
			If oPolygon IsNot Nothing Then
				...
				Else
					System.Windows.Forms.MessageBox.Show("Boundary was not found" & vbCrLf & CStr(iTopoID) & ":" & CStr(miCurrentTopoDefID.ID), "frmTplnView - zzZoomPolygon", MessageBoxButtons.OK, MessageBoxIcon.Warning)
	...
	Private Sub zzColorSchemeShow()
		...
		If miCurrentTopoDefID.ID <> 0 Then
			...
			Dim fEditColorScheme As frmColorEditor = New frmColorEditor(TopoManager.enColorEditorMode.ColorScheme, True, iCurrentLanduseID, tColorScheme.ID)
   ...
   Private Function zzGetRowParcelNameKey(ByVal oViewRow As DataGridViewRow) As String
      ...
      Try
         oValue = oViewRow.Cells.Item(TplnParcel.msBlockFieldName).Value
         If oValue IsNot Nothing Then
            iBlock = DirectCast(oValue, Integer)
            oValue = oViewRow.Cells.Item(TplnParcel.msNameFieldName).Value
            If oValue IsNot Nothing Then
   ...
   Private Function zzGetRowLanduseID(ByVal oViewRow As DataGridViewRow) As Integer
      Dim oValue As System.Object
      Try
         oValue = oViewRow.Cells.Item(TplnParcel.msLanduseIDFieldName).Value
         If oValue IsNot Nothing Then
            Return DirectCast(oValue, Integer)
         Else
            Return 0
   ...
   Private Function zzGetPolygon(ByVal iTopoID As Integer) As Autodesk.Gis.Map.Topology.Polygon
      If moTopoModel Is Nothing Then
         moTopoModel = DMAcadExt.AcadMapApp.GetTopology(zzGetCurrentTopoName())
      End If
      ...
      If mbTopologyExists Then
         ...
         If sTopoName IsNot Nothing Then
            ...
            If bHideForm Then
               Me.Hide()
            End If
            MessageBox.Show(sTopoName, "01_601")
            Do
   ...
   Private Sub zzSetDataGridColumns()
      ...
      'If moHiddenColumns Is Nothing Then
      '	moHiddenColumns = New Dictionary(Of Integer, Integer)
      'End If
      ...
      Try
         ...
         If oDataResource IsNot Nothing Then
            '	System.Windows.Forms.MessageBox.Show(CStr(oDataResource.ChildCount) & ":" & CStr(oDataResource.ObjectNo) & ":" & CStr(oDataResource.ItemCount) & vbCrLf & CStr(miResourceTheme) & ":" & CStr(10 + miColumnSetIndex), "01_590")
            If (moHiddenColumns IsNot Nothing) Then
               '  System.Windows.Forms.MessageBox.Show(moHiddenColumns.Count.ToString() & ":" & Me.dgvMain.Columns.Count.ToString(), "07_120")
            End If
            For Each oColumn As DataGridViewColumn In Me.dgvMain.Columns
               Try
                  ...
                  '	DMAcadExt.AcadDocument.WriteMessage(zzGetText(oColumn.Index, 10 + miColumnSetIndex))
   ...
   Private Sub zzDispParcels()
      ...
      If moCurrentDataView IsNot Nothing AndAlso mdicMapThemes.TryGetValue(DMAcadExt.enMapTheme.Parcels, tMapThemeData) Then
   ...
   Private Sub zzDispExpros()
      ...
      If moCurrentDataView IsNot Nothing Then
         miMapTheme = DMAcadExt.enMapTheme.Exprop
   ...
   Private Sub zzDispLots(ByVal iTopoPurpose As DMAcadExt.enTopoPurpose)
      ...
      If moCurrentDataView IsNot Nothing AndAlso mdicMapThemes.TryGetValue(miMapTheme, tMapThemeData) Then
   ...
   Private Sub zzDispPolygons(ByVal iOverlayIndex As DMAcadExt.enOverlayIndex)
      ...
      End If
      msAreaFieldName = TplnParcel.msPgonAreaFieldName
   End Sub
   ...
   Private Sub tcbData_SelectedIndexChanged(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles tcbData.SelectedIndexChanged
      ...
      Try
         ...
         Me.dgvMain.DataSource = moCurrentDataView
         '  System.Windows.Forms.MessageBox.Show(CStr(Me.dgvMain.Columns.Count) & vbCrLf & CStr(miColumnSetIndex) & vbCrLf & CStr(moCurrentDataView.Count), "01_431")
   ...
   Private Sub zzSetData()
      ...
      Else
         ...
         mbPolygons = False
         '	System.Windows.Forms.MessageBox.Show(DMAcadExt.DMApp.AppID.ToString() & vbCrLf & CStr(oItemData.ListIndex), "01_521")
         Select Case DMAcadExt.DMApp.AppID
            Case DMAcadExt.enApplications.Taba
   ...
   Private Sub zzExecTaba(ByVal iIndex As Integer)
      ...
      mbMarked = False
      ' System.Windows.Forms.MessageBox.Show(iIndex.ToString(), "01_574a")
      Select Case iIndex
         Case 0
   ...
   Private Sub zzDispExproOverlay()
      ...
      miMapTheme = DMAcadExt.enMapTheme.ExproOverlay
      miColumnSetIndex = 14
   ...
   Private Sub zzSetInitTable()
      ...
      Select Case mtInitMapThemeData.MapThemeID
         ...
         Case DMAcadExt.enMapTheme.LotApproved
            iInitIndex = 1
         Case DMAcadExt.enMapTheme.Exprop
            iInitIndex = 21
   ...
   Private Sub tsmiParcel_Click(sender As System.Object, e As System.EventArgs) Handles tsmiParcel.Click
   ...
   Private Sub tsmiLot_Click(sender As System.Object, e As System.EventArgs) Handles tsmiLot.Click
      Select Case miMapTheme
         Case DMAcadExt.enMapTheme.ParcelsXApproved
            zzZoomLot(True)
      End Select
