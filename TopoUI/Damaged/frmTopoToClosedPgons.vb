...
#End Region
	...
	Public Sub New(tSourceMapThemeData As DMAcadExt.MapThemeData, tDissolveMapThemeData As DMAcadExt.MapThemeData)
		...
		InitializeComponent()
      '   MessageBox.Show(tSourceMapThemeData.TopoName & ":" & tDissolveMapThemeData.TopoName, "04_400")
   ...
   Private Sub zzCheckSourceTopo(bLockDoc As Boolean, ByVal bMsg As Boolean)
      mtSourceTopoRes = TopoManager.TopoCreator.CheckTopo(mtSourceMapThemeData.TopoName, bLockDoc, bMsg)
      zzDispSourceTopoOK()
      mtSourceTopoWARes = TopoManager.TopoCreator.CheckTopo(mtSourceMapThemeData.LineTopoName, bLockDoc, bMsg)
      zzDispSourceTopoWAOK()
   End Sub
   Private Sub zzCheckDissolveTopo(bLockDoc As Boolean, ByVal bMsg As Boolean)
      mtDissolveTopoRes = TopoManager.TopoCreator.CheckTopo(msDissolveTopoName, bLockDoc, bMsg)
   ...
   Private Sub zzCreateDissolveTopology()
      ...
      Else
         ...
         '    System.Windows.Forms.MessageBox.Show(sSourceTopoName & vbCrLf & sAttribExpr & vbCrLf & msDissolveTopoName & vbCrLf & mtDissolveMapThemeData.ClosedPgonsLayer & vbCrLf & mtDissolveMapThemeData.CentroidBlock & vbCrLf & mtDissolveMapThemeData.CentroidLayer, "06_008")
   ...
   Private Sub zzCreateDissolveTopologyNew()
      ...
      Else
         ...
         TopoManager.TPlanGraph.TplnBlock.Initialize(mtDissolveMapThemeData)
         '   System.Windows.Forms.MessageBox.Show(sSourceTopoName & vbCrLf & sAttribExpr & vbCrLf & msDissolveTopoName & vbCrLf & mtDissolveMapThemeData.LinkLayer & vbCrLf & mtDissolveMapThemeData.ClosedPgonsLayer & vbCrLf & mtDissolveMapThemeData.CentroidBlock & vbCrLf & mtDissolveMapThemeData.CentroidLayer, "06_008")
   ...
   Private Sub zzUpdateBlocksDic()
      ...
      If moBlockTable IsNot Nothing Then
         For iRowIndex As Integer = 0 To moBlockTable.Rows.Count - 1
            ...
            iBlockNo = DirectCast(oRow.Item(TopoManager.TPlanGraph.TplnParcel.msBlockFieldName), Integer)
            iBlockNoAdd = DirectCast(oRow.Item(TopoManager.TPlanGraph.TplnParcel.msBlockAddFieldName), Integer)
            ...
            If oBlock IsNot Nothing Then
               ...
               Try
                  oBlock.BlockStatusStr = CType(oRow.Item(TopoManager.TPlanGraph.TplnParcel.msBlockStatusFieldName), String)
               ...
               End Try
               Try
                  If Not IsDBNull(oRow.Item(TopoManager.TPlanGraph.TplnParcel.msBlockIsAnalyticFieldName)) Then
                     oBlock.IsAnalytic = Convert.ToBoolean(DirectCast(oRow.Item(TopoManager.TPlanGraph.TplnParcel.msBlockIsAnalyticFieldName), Integer))
                  End If
               Catch oEx As Exception
   ...
   Private Sub zzFillBlockGrid()
      ...
      If moBlockTable IsNot Nothing Then
         ...
         moBlockView = New DataView(moBlockTable, String.Empty, TPlanGraph.TplnParcel.msBlockFieldName & "," & TPlanGraph.TplnParcel.msBlockAddFieldName, DataViewRowState.CurrentRows)
   ...
   Private Sub zzTopoToClosedPgonsOld(tMapThemeData As DMAcadExt.MapThemeData, bDissolve As Boolean, bExteriorRingOnly As Boolean)
      ...
      If Not String.IsNullOrEmpty(tMapThemeData.ClosedPgonsLayers) Then
         ...
         If bCurrentLayerOK Then
            ...
            End If
         Else
            MessageBox.Show("Layer '" & tMapThemeData.ClosedPgonsLayers & "' Error", "03_122")
         End If
         DMAcadExt.AcadTransaction.CloseModelSpace()
         DMAcadExt.AcadTransaction.Terminate()
         DMAcadExt.AcadDocument.CloseMessage()
   ...
   Private Sub zzUpdateAfterDissTopo()
      ...
      TopoManager.TPlanGraph.TplnProject.LoadParcelsNew()
   ...
   Private Sub zzCheckTopo(ByVal bMsgBox As Boolean)
      Dim tTopoRes As DMAcadExt.TopoRes = TopoManager.TopoCreator.CheckTopo(msDissolveTopoName, True, bMsgBox)
   End Sub
