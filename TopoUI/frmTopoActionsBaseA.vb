...
Option Strict On
Imports TopoManager
Public Class frmTopoActionsBaseA
	...
	Private WithEvents tsiBuildTopo As System.Windows.Forms.ToolStripItem
	Private WithEvents tsiKillTopo As System.Windows.Forms.ToolStripItem
	Private WithEvents tsiShowTopo As System.Windows.Forms.ToolStripItem
	...
	Private Sub zzMyInitializeComponent()
		...
		Me.tsiBuildTopo = New System.Windows.Forms.ToolStripMenuItem
		Me.tsiKillTopo = New System.Windows.Forms.ToolStripMenuItem
		Me.tsiShowTopo = New System.Windows.Forms.ToolStripMenuItem
		...
		'
		With Me.cmdShow
			.Location = New System.Drawing.Point(370, 4)
			.Name = "cmdShow"
		...
		With Me.cmdEraseTopoGeometria
			.Location = New System.Drawing.Point(406, 4)
		...
		With Me.cmdCopyFromOverlay
			.Location = New System.Drawing.Point(442, 4)
		...
		'
		With Me.cmdBuild
			.Location = New System.Drawing.Point(298, 4)
			.Name = "cmdBuild"
		...
		'
		With Me.cmdKill
			.Location = New System.Drawing.Point(334, 4)
			.Name = "cmdKill"
		...
		With Me.cmdToClosedPgons
			.Location = New System.Drawing.Point(478, 4)
		...
		'cmsMain
		'
		Me.cmsMain.AllowDrop = True
		Me.cmsMain.Items.AddRange(New System.Windows.Forms.ToolStripItem() {Me.tsiBuildTopo, Me.tsiKillTopo, Me.tsiShowTopo})
		Me.cmsMain.Name = "cmsMain"
		Me.cmsMain.RightToLeft = System.Windows.Forms.RightToLeft.Yes
		Me.cmsMain.Size = New System.Drawing.Size(95, 26)
		Me.cmsMain.Text = "aaaaaaaaaa"
	...
	Protected Sub OnNew()
		...
		If oTopoDef IsNot Nothing Then
			...
			Me.cmdPrepare.Enabled = oTopoDef.PrepareEnabled
		End If
	...
	Private Sub zzSetCleanupErrPoints()
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActions - zzSetCleanupErrPoints")
		End Try
	End Sub
	...
	Private Sub lblTopo_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs)
		...
		Try
			...
			'   zzTopoLayersIsOn()
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & oEx.StackTrace, "frmTopoActions - lblTopo_Click")
		End Try
	...
	Private Sub zzSetTopoErr()
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActions - zzSetTopoErrPoints")
		End Try
	End Sub
	...
	Private Sub nudErrors_ValueChanged(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles nudErrors.ValueChanged
		...
		Else
			...
			Catch oEx As Exception
				System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & oEx.StackTrace, "frmTopoActions - nudErrors_ValueChanged+")
			End Try
		End If
	End Sub
	Private Sub zzZoomPoint(ByVal oPoint As DMAcadExt.TPlnPoint)
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActions - zzZoomPoint")
		End Try
	End Sub
		...
		If oTopoDef IsNot Nothing Then
			...
			Else
				If Information.IsNumeric(Me.txtTolerance.Text) Then
					...
					Try
						TopoCreator.CreateTopology(DMAcadExt.DMApp.AppID, oTopoDef, Me.chkIgnoreIncompleteArea.Checked, Me.chkHighlightSliver.Checked, dTolerance, False)
					Catch oEx As Exception
						System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActions - zzCreateTopo_01")
					End Try
				Else
					Beep()
				End If
	...
	Private Sub cmdZoomMsg_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdZoomMsg.Click
		...
		Try
			...
			End If
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActionsM - cmdZoomMsg_Click")
		End Try
	End Sub
	Private Sub cmdClearMsg_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles cmdClearMsg.Click
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message, "frmTopoActionsM - cmdClearMsg_Click")
		End Try
	End Sub
	...
	Private Sub zzToClosedPgons()
		...
		If oTopoDef IsNot Nothing Then
			...
			If bCurrentLayerOK Then
				Dim oTopoModel As Autodesk.Gis.Map.Topology.TopologyModel = TopoManager.TopoCreator.GetOpenedTopology(oTopoDef.Name, Autodesk.Gis.Map.Topology.OpenMode.ForRead, True)
	...
	Protected MustOverride Sub OpenProjectData()
	...
	Protected Sub InitTabProjectData()
			...
			For iIndex As Integer = 0 To iControlsUB
				...
				Select Case doProjectData.ControlTypes(iIndex)
					...
					Case Is >= ProjectData.enControlType.ComboBox
						...
						If doProjectData.ControlTypes(iIndex) = ProjectData.enControlType.ComboBoxColorSet Then
							TPlanGraph.TplnProject.FillColorSets(Me.cmbProjectData(iIndex), doProjectData.dmItem(iIndex).IntValue)
						End If

						If doProjectData.ControlTypes(iIndex) = ProjectData.enControlType.ComboBoxScale Then
							TPlanGraph.TplnProject.FillScales(Me.cmbProjectData(iIndex))
						End If
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message & vbNewLine & sTest, "frmTopoActionsB - zzInitTabProjectData")
		End Try
		DMAcadExt.AcadTransaction.Terminate()
		DMAcadExt.AcadDocument.Unlock()
		SetProjectData()
		...
		If iAcadActionUB >= 0 OrElse tCleanupOptions.HasAction Then
			...
			If bCurrentLayerOK Then
				...
				If iAcadActionUB >= 0 Then
					...
					Catch oEx As Exception
						System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & oEx.StackTrace, "frmTopoActions - zzCleanup")
					End Try
	...
	Protected Class ReportItem
		...
		Public Sub New(ByVal iListIndex As TPlServerDB.enResourceTheme, ByVal sListDispData As String, ByVal iOptions As TopoManager.TPlanGraph.enDataOptions, ByVal bExcel As Boolean, ByVal bAcad As Boolean, ByVal bAcadTable As Boolean)
	...
	Private Sub tmrInactive_Tick(ByVal sender As Object, ByVal e As System.EventArgs) '''''''''''''''''''''''' Handles tmrInactive.Tick
	...
	Private Sub dfEditColorScheme_FormClosed(ByVal oSender As System.Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles dfEditColorScheme.FormClosed
		...
		Catch oEx As Exception
			System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & oEx.StackTrace, "frmTopoActions - dfEditColorScheme_FormClosed")
		End Try
	End Sub
	...
	Private Sub dfSelectLanduse_FormClosing(ByVal oSender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles dfSelectLanduse.FormClosing

	End Sub

	Private Sub tsiBuildTopo_Click(ByVal oSender As System.Object, ByVal e As System.EventArgs) Handles tsiBuildTopo.Click
		Dim oMyExporter As Autodesk.Gis.Map.ImportExport.Exporter = Autodesk.Gis.Map.HostMapApplicationServices.Application.Exporter
		Dim oRes As Autodesk.Gis.Map.ImportExport.ExportResults
		Dim sFileName As String = "E:\aWork\TestMapExp\p6.shp"
		Dim oTopoDef As DMAcadExt.TopoDef = zzLoadTopoDef()
		...
		Try
			oMyExporter.Init("SHP", sFileName)
			'		oMyExporter.LoadExportFormat()	'FileOneEntityType
			oMyExporter.SetStorageOptions(Autodesk.Gis.Map.ImportExport.StorageType.FileOneEntityType, Autodesk.Gis.Map.ImportExport.GeometryType.Polygon, "")
			oMyExporter.SetExportFromPolygonTopology(False, sTopoName)
			'	oMyExporter.ExportAll = True
			colExpressionTarget = oMyExporter.GetExportDataMappings()
			...
			oMyExporter.SetExportDataMappings(colExpressionTarget)
			'	oMyExporter.ExportFromPolygonTopology(False, sTopoName)
			oRes = oMyExporter.Export()
		...
		'oMyExporter.SetExportFromPolygonTopology()
		'oRes = oMyExporter.Export()
	End Sub
End Class