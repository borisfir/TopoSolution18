...
#Region "Panel5_Declarations"
   ...
   Private moCheckActionsTable As DataTable
...
#End Region
   ...
   Private Sub zzInitPanel5()
      ...
      With Me.cmdExec
         .Image = Global.TopoUI.My.Resources.Resources.Run15Tr
         .Location = New System.Drawing.Point(36, 4)
         .Name = "cmdExec"
   ...
   Private Sub dgvCheckActions_RowEnter(ByVal oSender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
      ...
      Dim oDataRow As DataRow = moCheckActionsTable.Rows.Item(e.RowIndex)
   ...
   Private Sub zzExecOld()
      ...
      bRes = TopoManager.TPlanGraph.TplnProject.LoadParcelsNew()
      If bRes Then

         TPlanGraph.TplnProject.CalculateParcels(bMerge, bUnion, bFDO_Overlay, bApproved, bProposed)
      End If
      '		qqqqqqqqqqqqqqqqqqqqqqqqqq

      If bRes Then ''''''''''''gggggggggggg
         '**************************************************** _429
         TPlanGraph.TplnProject.UpdateParcelTable(bMerge, bUnion, bFDO_Overlay, bApproved, bProposed)      '''''''''''''''''''''''''''''   02/06/09
   ...
   Private Sub mfMessages_FormClosed(ByVal oSender As System.Object, e As System.Windows.Forms.FormClosedEventArgs) Handles mfMessages.FormClosed
      Me.Visible = True
   End Sub
   Private Sub zzSetErrValue(iActionID As Integer, iErrNumber As Integer)
      ...
      End If
   End Sub
   Private Sub zzSetErrValue(iActionID As Integer)
      Dim iErrNumber As Integer = DMAcadExt.AppMessages.GetMessagesCount(ptMapThemeData.MapThemeID, iActionID)
      Dim oRow As System.Data.DataRow = moCheckActionsTable.Rows.Find(iActionID)
      If oRow IsNot Nothing Then
         oRow.Item("ErrNumber") = Math.Max(DMCommon.Functions.CIntN(oRow.Item("ErrNumber")), iErrNumber)
      End If
...
#Region "CleanupProcedures"
   ...
   Private Sub nudErrors_ValueChanged(ByVal oSender As System.Object, ByVal e As System.EventArgs)    'Handles nudErrors.ValueChanged
      ...
      'DMCommon.Debug.MsgBox("12_461", iErrIndex, iMax)
...
#End Region
   ...
   Private Sub zzLoadCheckData()
      ...
      Dim oPointArray As DMAcadExt.TplnPointArray = New DMAcadExt.TplnPointArray()
      Dim sComText As String = "SELECT TOP (100) PERCENT dbo.CheckActionsExt.* FROM dbo.CheckActionsExt WHERE (CheckType = 1) AND (MapThemeID = " & Convert.ToString(ptMapThemeData.MapThemeID) & ") ORDER BY ActionOrder"
      '      sSelectComText = "SELECT TopologyType,Step,ActionNo,ActionID,Tolerance FROM CleanupActions WHERE (CleanupActions.TopologyType=" & Convert.ToString(ptMapThemeData.CleanupType) & ") ORDER BY CleanupActions.Step,CleanupActions.ActionNo"
      ...
      If oDataReader IsNot Nothing Then
         Do While oDataReader.Read
            ...
            oNewRow.Item("ActionID") = oDataReader.GetInt32(2)
            oNewRow.Item("ActionName") = oDataReader.GetString(5)
            If Not oDataReader.IsDBNull(6) Then
               oNewRow.Item("ActionDescription") = oDataReader.GetString(6)
            End If
   ...
   Private Sub zzCreateCheckActionsTable()
      ...
      moCheckActionsTable = New DataTable("CheckActions")
      ...
      moCheckActionsTable.Columns.Add(oDataColumn)
      Dim oPrimaryKey() As DataColumn = {moCheckActionsTable.Columns.Item("ActionID")}
   ...
   Private Sub zzCheckTopo(bLockDoc As Boolean, ByVal bMsgBox As Boolean)
      ...
      DMAcadExt.AcadTransaction.CloseModelSpace()
      '  DMCommon.Debug.MsgBox("12_440", colBlockRefs.Count, ptMapThemeData.CentroidBlocks, ptMapThemeData.CentroidLayers)
      mtTopoRes = TopoCreator.CheckTopo(ptMapThemeData.TopoName, bLockDoc, bMsgBox, ptMapThemeData.MapThemeID, colBlockRefs)
   ...
   Private Sub zzCheckTopoWA(bLockDoc As Boolean, ByVal bMsgBox As Boolean)
      ...
      mtTopoResWA = TopoCreator.CheckTopo(ptMapThemeData.LineTopoName, bLockDoc, bMsgBox, ptMapThemeData.MapThemeID)
   ...
   Private Sub zzCheckParcels()
      ...
      ' iErrNumber = DMAcadExt.AppMessages.GetMessagesCount(ptMapThemeData.MapThemeID, 22)
      ' DMCommon.Debug.MsgBox("12+421", iErrNumber, ptMapThemeData.MapThemeID)
   ...
   Private Sub zzCheckUDParcels()
      Dim iMapTheme As DMAcadExt.enMapTheme = ptMapThemeData.MapThemeID
      ...
      If oParcelTopology IsNot Nothing Then
         ...
         For Each oPolygon In colParcelPgons
            ...
            If String.IsNullOrEmpty(sParcelName) Then
            Else
               oIdenticalNames.Add(sParcelName, True, oPolygon.ID)
            End If
         Next
         For Each oQueue As Queue(Of Integer) In oIdenticalNames.DoubleValues.Values

            For Each iTopoID As Integer In oQueue.ToArray()
               oPolygon = oParcelTopology.GetPolygon(iTopoID)
               If oPolygon IsNot Nothing Then
                  DMAcadExt.AppMessages.AddMessage(True, oPolygon.Centroid.X, oPolygon.Centroid.Y, "", DMCommon.dmMessages.Message(308, Name), False, iMapTheme, 11)
               End If
            Next
         Next
      End If
   ...
   Private Sub zzCheckLots(ByVal iTopoPurpose As DMAcadExt.enTopoPurpose)
      ...
      Dim bProposed As Boolean = False
      '   Dim iErrNumber As Integer
      '  DMCommon.Debug.MsgBox("12_460", ptMapThemeData.MapThemeID)
      TopoManager.TPlanGraph.TplnLot.Initialize(ptMapThemeData)
      TopoManager.TPlanGraph.TplnProject.InitRegionDic()


      bRes = TopoManager.TPlanGraph.TplnProject.LoadLots(iTopoPurpose)
   ...
   Private Sub zzCheckMapPoints()
      ...
      If oFragmentTopology IsNot Nothing Then
         ...
         For Each oNode In colNodes
            If oNode.Entity.IsNull Then
               DMCommon.Debug.MsgBox("09_110", "Node.Entity.IsNull", oNode.Location)

            Else

               sPointName = DMAcadExt.AcadTransaction.GetAttribText(oNode.Entity, True, bAcadPoint, "")
               If String.IsNullOrEmpty(sPointName) Then
               Else
         ...
         For Each oQueue As Queue(Of Integer) In oIdenticalNames.DoubleValues.Values
            ...
            For Each iTopoID As Integer In oQueue.ToArray()
               ...
               If oNode IsNot Nothing Then
                  ...
                  DMAcadExt.AppMessages.AddMessage(True, oNode.Location.X, oNode.Location.Y, "", DMCommon.dmMessages.Message(308, Name), False, iMapTheme, 99)
               End If
            Next
         Next
      End If
   ...
   Private Sub zzSetErrValues()
      ...
      '  DMCommon.Debug.MsgBox("12+421a", iErrNumber, ptMapThemeData.MapThemeID, mtTopoRes.TopoExists, IsDoneA)
      If Me.IsDoneA Then
         Me.doaLabelCheck(5).Checked = True
      End If
   End Sub
   Private Function zzGetErrMessagesCount() As Integer
      Return DMAcadExt.AppMessages.GetMessagesCount(ptMapThemeData.MapThemeID)
   End Function
   ...
   Public ReadOnly Property IsDoneA_251217 As Boolean
      Get
         ...
         Select Case ptMapThemeData.TopoPriority
            ...
            Case Else
               Return False
         End Select
         DMCommon.Debug.MsgBox("12_430", ptMapThemeData.TopoPriority, bTopologyExists, zzGetErrMessagesCount())
         Return bTopologyExists AndAlso (zzGetErrMessagesCount() = 0)
   ...
   Private Sub mfTplnView_VisibleChanged(sender As Object, e As EventArgs) Handles mfTplnView.VisibleChanged
