...
#End Region
	...
	Public Sub New(tMapThemeData As DMAcadExt.MapThemeData)
		...
		MessageBox.Show(Me.Text & vbCrLf & Me.Name & vbCrLf & MyBase.Name, "15_001")
		...
		'	DMAcadExt.AcadDocument.TestAcadDoc("265Before doc Lock")
		DMAcadExt.AcadDocument.DocLock(Autodesk.AutoCAD.ApplicationServices.DocumentLockMode.Read, True)

		'	DMAcadExt.AcadDocument.TestAcadDoc("266After doc Lock")
		zzCheckTopo(False, False)

		zzCheckTopoWA(False, False)
		'DMAcadExt.AcadDocument.TestAcadDoc("267After doc Lock")
	...
	End Sub
	Private Sub zzRefreshAcadData160613()
	...
	Private Sub zzCleanupBBB(ByVal iCleanupIndex As Integer, ByVal bFix As Boolean)
	...
	Private Sub zzLoadData(iCleanupIndex As Integer)
		...
		moCurrentView(iCleanupIndex) = New System.Data.DataView(moCleanupActionsTable, "Step=" & sStepNo & "", "", DataViewRowState.CurrentRows)
						...
						If ptMapThemeData.IsNotEmpty Then
							Try
								...
								If iCleanupIndex = 0 Then
									sBaseLayers = ptMapThemeData.LinkLayers
									colLinks = New Autodesk.AutoCAD.DatabaseServices.ObjectIdCollection()
									'	MessageBox.Show(CStr(iCleanupIndex) & vbCrLf & ptMapThemeData.LineLinkLayers & vbCrLf & CStr(colLinks.Count), "05_013")
								ElseIf iCleanupIndex = 1 Then
									sBaseLayers = String.Empty
	...
	Private Sub zzCreatePgonSet()
		...
		Dim oPgonSetCreator As FDO.PgonSetCreator = New FDO.PgonSetCreator()
		...
		Dim dicPolygons As System.Collections.Generic.IDictionary(Of ObjectId, Polyline) = DMAcadExt.AcadTransaction.GetAcadPolylinesDic("1602", OpenMode.ForWrite)	'"pclp004"
		...
		oEditor.WriteMessage("Pgon Count: " & CStr(dicPolygons.Count) & vbCrLf)
		oPgonSetCreator.AddPolylines(dicPolygons)

		'''''''''''''		bCurrentLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(ptMapThemeData.MPgonLayers, DMAcadExt.DMApp.AppID, DMAcadExt.enLayerFunction.Default, True, True, False)

		oPgonSetCreator.AddBlocks(dicCentroids)
		TopoManager.TPlanGraph.TplnParcel.Initialize(ptMapThemeData)
		oPgonSetCreator.Calculate()
		DMAcadExt.AcadTransaction.CloseModelSpace()
		DMAcadExt.AcadTransaction.Terminate()
		DMAcadExt.AcadDocument.Unlock()
	End Sub
	...
	Private Sub zzCreateTopo()
		...
		If Information.IsNumeric(Me.txtTolerance.Text) Then
			...
			Try
				'	MessageBox.Show(ptMapThemeData.LinkLayers & vbCrLf & ptMapThemeData.LineLinkLayers, "03_324")
				mtTopoRes = TopoCreator.CreateTopology(ptMapThemeData.TopoName, ptMapThemeData.LinkLayers, "", ptMapThemeData.CentroidBlocks, ptMapThemeData.CentroidLayers, bCreateCentroids, Me.chkHighlightSliver.Checked, dTolerance)
				zzDispTopoOK()
	...
	Private Sub zzCreateTopoWA()
		...
		If Information.IsNumeric(Me.txtToleranceWA.Text) Then
			...
			Try
				mtTopoResWA = TopoCreator.CreateTopology(ptMapThemeData.LineTopoName, ptMapThemeData.LinkLayers, ptMapThemeData.LineLinkLayers, ptMapThemeData.CentroidBlocks, ptMapThemeData.CentroidLayers, bCreateCentroids, Me.chkHighlightSliverWA.Checked, dTolerance)
				zzDispTopoOK_WA()
	...
	Private Sub zzGetStatisticsA()
		...
		mtMapThemeInfo = DMAcadExt.AcadTransaction.GetCPStatistics(ptMapThemeData)
		...
		Me.txtClosedPgonsCount.Text = CStr(mtMapThemeInfo.MPgonsCount)
		Me.txtPgonCount.Text = CStr(mtMapThemeInfo.MPgonsCount)

		If mtMapThemeInfo.PgonSetIsCorrect Then
	...
	Public Overrides ReadOnly Property IsDone As Boolean
		Get
			'System.Windows.Forms.MessageBox.Show(CStr(mtTopoRes.IsComplete) & ":" & CStr(mtTopoResWA.IsComplete), "zzIsDone")
