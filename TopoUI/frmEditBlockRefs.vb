...
Public Class frmEditBlockRefs
   ...
   Private moCurrentAcadBlock As DMAcadExt.AcadBlock
	Private mcolCurrentBlockRefIDs As ObjectIdCollection
   ...
   Private moMainDataView As System.Data.DataView
	Private moDfltComparer As IComparer
	...
	Private Class DfltComparer
		Implements IComparer

		Public Function Compare(x As System.Object, y As System.Object) As Integer Implements IComparer.Compare
			'DMCommon.Debug.MsgBox("13_047XX", x.GetType(), y.GetType())
			Return 0
		End Function
	End Class
	Public Sub New()
      ...
      midgvMainLocationY = Me.dgvMain.Location.Y
		...
		moDfltComparer = New DfltComparer()
		'''''''''''''''''''''''Me.dgvMain.Sort(moDfltComparer)
		Try
      ...
      Dim bLock As Boolean
		...
		If oDyn IsNot Nothing Then
         ...
         moCurrentAcadBlock = DirectCast(oDyn, DMAcadExt.AcadBlock)
			...
			Me.chkAdditionalBlock.Enabled = moCurrentAcadBlock.AdditionalBlock IsNot Nothing OrElse moCurrentAcadBlock.PrimaryBlock IsNot Nothing
         mbAdditionalBlockExists = Me.chkAdditionalBlock.Checked
			'	DMCommon.Debug.MsgBox("13_047c", dgvMain.SortedColumn, mbAdditionalBlockExists)
			If mbAdditionalBlockExists Then
         ...
         End If
			...
			If moCurrentAcadBlock.HasAttributeDefinitions Then
				...
				Loop
			End If
			'	DMCommon.Debug.MsgBox("13_047D", dgvMain.SortedColumn, mbAdditionalBlockExists)
         ...
         End If
			...
			moMainDataView = New System.Data.DataView(moMainTable, String.Empty, msObjectIDFieldName, Data.DataViewRowState.CurrentRows)
			'   System.Windows.Forms.MessageBox.Show(CStr(moMainTable.Rows.Count) & ":" & CStr(moMainDataView.Count), "21_099")
			'MCommon.Debug.MsgBox("13_047DK", dgvMain.SortedColumn)
			dgvMain.DataSource = moMainDataView
			'	MCommon.Debug.MsgBox("13_047DL", dgvMain.SortedColumn)
   ...
   End Sub
   Private Sub zzFill()
		If moCurrentAcadBlock Is Nothing Then
			DMCommon.Debug.MsgBox("BM001", "moCurrentAcadBlock Is Nothing")
		End If
		moCurrentAcadBlock.LoadAllReferences()


		'Dim colBlockRefIDsAAAA As ObjectIdCollection = moCurrentAcadBlock.BlockRefObjIds
		mcolCurrentBlockRefIDs = moCurrentAcadBlock.BlockRefObjIds
      ...
      End If
		...
		For Each tBlockRefObjID As ObjectId In mcolCurrentBlockRefIDs
			...
			End Try
			'DMCommon.Debug.MsgBox("13_047FC", dgvMain.SortedColumn
			'DMCommon.Debug.MsgBox("13_047p", iAttribRefUB, oBlockRef.Name)
   ...
   End Sub
	...
	Private Sub cmdDispSelSet_Click(oSender As System.Object, e As System.EventArgs) Handles cmdDispSelSet.Click
		...
		End If
		DMCommon.Debug.MsgBox("13_135", taAcObjID.GetUpperBound(0))
   ...
   End Sub
	...
	Private Sub cmdImportExcel_ClickOld(oSender As System.Object, e As EventArgs) 'Handles cmdImportExcel.Click
		Dim iRowIndex As Integer = -1
		Dim iColumnIndex As Integer = -1
		Dim oValue As System.Object
		Dim oRow As DataGridViewRow
		'  Dim oCell As DataGridViewCell
		DMCommon.Debug.MsgBox("AAA1", "")
		If DMCommon.ExcelImport.OpenExcelApp() Then

			DMCommon.Debug.MsgBox("L0", "")
			Dim oaValues As System.Object(,) = DMCommon.ExcelImport.GetSelectionValue()
			Dim oCurrentCell As System.Windows.Forms.DataGridViewCell = Me.dgvMain.CurrentCell
			DMCommon.Debug.MsgBox("L1", oaValues.GetUpperBound(0), oaValues.GetUpperBound(1))
			If oCurrentCell IsNot Nothing AndAlso oaValues IsNot Nothing Then
				...
				DMCommon.Debug.MsgBox("L2", oCurrentCell.RowIndex, oCurrentCell.ColumnIndex)
	...
	Private Sub cmdImportExcel_Click(oSender As System.Object, e As EventArgs) Handles cmdImportExcel.Click
      ...
      Dim oRow As DataGridViewRow
		'  Dim oCell As DataGridViewCell
		DMCommon.Debug.MsgBox("AAA1", "ImportExcel")





		Dim oaValues As System.Object(,) = DMCommon.Debug.ExcelLog.GetSelectionValue()
		Dim oCurrentCell As System.Windows.Forms.DataGridViewCell = Me.dgvMain.CurrentCell
			DMCommon.Debug.MsgBox("L1", oaValues.GetUpperBound(0), oaValues.GetUpperBound(1))
			If oCurrentCell IsNot Nothing AndAlso oaValues IsNot Nothing Then
				'  System.Windows.Forms.MessageBox.Show(CStr(oCurrentCell.RowIndex) & ":" & CStr(oCurrentCell.ColumnIndex), "10_002")
   ...
   End Sub
	...
	Private Sub cmdDelete_Click(oSender As System.Object, e As EventArgs) Handles cmdDelete.Click
		Dim colSelectedRows As System.Windows.Forms.DataGridViewSelectedRowCollection = Me.dgvMain.SelectedRows
		Dim tAcObjID As ObjectId
		Dim colSelectionBlockRefs As ObjectIdCollection = New ObjectIdCollection()
		For Each oGridRow As System.Windows.Forms.DataGridViewRow In colSelectedRows
			tAcObjID = zzGetRowObjectIDNew(oGridRow)

			If Not tAcObjID.IsNull Then
				colSelectionBlockRefs.Add(tAcObjID)
			End If
		...
		DMAcadExt.AcadTransaction.Start()
		DMAcadExt.AcadTransaction.OpenModelSpace(Autodesk.AutoCAD.DatabaseServices.OpenMode.ForWrite, True)
		DMAcadExt.AcadTransaction.EraseDBObjects(colSelectionBlockRefs)
		DMAcadExt.AcadTransaction.CloseModelSpace()
	...
	Private Sub txtValue_TextChanged(oSender As System.Object, e As EventArgs) Handles txtValue.TextChanged
