...
#End Region
   ...
   Private Structure TopoSelection
      ...
      Public Sub New(iCount As Integer, iMinIndex As Integer, iMaxIndex As Integer)
         ...
         End If
         '  DMCommon.Debug.MsgBox("08_422", miMinIndex, miMaxIndex, mbOpened)
      End Sub
   ...
   End Sub
	...
	Private Sub zzCheckClosedPgons(bLockDoc As Boolean, ByVal bMsg As Boolean)
		Dim tTopoRes As DMAcadExt.TopoRes = TopoManager.TopoCreator.CheckTopo(mtMapThemeData.LineTopoName, bLockDoc, bMsg, mtMapThemeData.MapThemeID)
		Dim tMapThemeInfo As DMAcadExt.MapThemeInfo = DMAcadExt.AcadTransaction.GetStatistics(mtMapThemeData)
		zzDispClosedPgonsOK(tMapThemeInfo)
	End Sub
   ...
   End Sub
	Private Sub zzCheckBlueLineTopo(bLockDoc As Boolean, ByVal bMsg As Boolean)
		If mtBlueLineMapThemeData.SourceMapThemeID <> DMAcadExt.enMapTheme.Undefined Then
	...
	Private Sub zzDispSourceTopoOK(tTopoRes As DMAcadExt.TopoRes)
		...
		zzSetPgonCount()
	End Sub
	Private Sub zzDispClosedPgonsOK(tMapThemeInfo As DMAcadExt.MapThemeInfo)
		'	Dim bOK As Boolean = tTopoRes.IsOK
		'	Me.lblSourceTopoExists.Visible = bOK
		'Me.txtSourcePgonCount.Enabled = bOK
		With lblSourceTopoName
			...
			End If
		End With
		Me.txtSourcePgonCount.Text = Convert.ToString(tMapThemeInfo.LinkCount)
		miSourcePolygonsCount = tMapThemeInfo.LinkCount
		zzSetPgonCount()
	End Sub
   ...
   Private Sub zzCreateLanduseTopology()
      ...
      Dim sLanduseTopoName As String = mtMapThemeData.DissolveTopoName
      Dim tLanduseLayerDef As DMAcadExt.AcadLayerDef = New DMAcadExt.AcadLayerDef(DMAcadExt.DMApp.AppID, DMAcadExt.enLayerFunction.LanduseK)
      DMAcadExt.AcadDocument.DocLock(Autodesk.AutoCAD.ApplicationServices.DocumentLockMode.ExclusiveWrite, True)
      DMAcadExt.AcadTransaction.Start()
      Dim bCurrentLayerOK As Boolean = DMAcadExt.AcadTransaction.SetCurrentLayer(tLanduseLayerDef, True, False, True, False)
   ...
   Private Sub cmdPaint_Click(ByVal oSender As System.Object, ByVal oEventArgs As System.EventArgs) Handles cmdPaint.Click
      ...
      '	Dim iTopoPurpose As DMAcadExt.enTopoPurpose = zzGetPaintTopoPurpose()
      Dim sPaintLayer As String = Nothing
      Dim sPLineLayer As String = Nothing
      ...
      zzSetCondition()
		...
		'DMCommon.Debug.MsgBox("08_561c", iTopoPurpose)
		'   MessageBox.Show(mtMapThemeData.MapThemeID.ToString() & vbCrLf & mtMapThemeData.TopoPurpose.ToString(), "09_560")
		If True OrElse iTopoPurpose <> DMAcadExt.enTopoPurpose.Undefined Then
			...
			''''''''	Dim sLotTopoName As String = mtMapThemeData.LineTopoName
			Dim sLanduseTopoName As String = mtMapThemeData.DissolveTopoName
			TopoManager.TPlanGraph.TplnProject.SetPaintLayers(mtMapThemeData.MapThemeID, iTopoPurpose, sPaintLayer, sPLineLayer)
			'  DMCommon.Debug.MsgBox("04_218a", sPaintLayer, sPLineLayer)
			'    Dim sLayer As String = DMAcadExt.AcadTransaction.GetCurrentLayer()
			Dim bCurrentLayerOK As Boolean = True

			Dim tPaintLayerDef As DMAcadExt.AcadLayerDef = TopoManager.TPlanGraph.TplnProject.PaintLayerDef(mtMapThemeData.MapThemeID)  ', iTopoPurpose
			Dim tPLineLayerDef As DMAcadExt.AcadLayerDef = TopoManager.TPlanGraph.TplnProject.DrawPlineLayerDef(mtMapThemeData.MapThemeID)  ', iTopoPurpose

			'MessageBox.Show(sPaintLayer & ":" & sLayer, "21_320")
			'DMCommon.Debug.MsgBox("08_561d", sPaintLayer)
			zzFillColorSchemesDic()

			Me.prbPaint.Visible = True
			If sPaintLayer IsNot Nothing Then
				If Not Me.chkLayerByLanduse.Checked Then
					DMAcadExt.AcadTransaction.CreateLayer(tPaintLayerDef, True)
					DMAcadExt.AcadTransaction.CreateLayer(tPLineLayerDef, True)
				End If
				If mtMapThemeData.MapThemeID = DMAcadExt.enMapTheme.LotApproved OrElse mtMapThemeData.MapThemeID = DMAcadExt.enMapTheme.LotProposed Then
					If rdbDissolve.Checked Then
						...
						zzPaintByLanduseTopo(iTopoPurpose, tPaintLayerDef, tPLineLayerDef, Me.chkLayerByLanduse.Checked, Me.chkBypassLine.Checked, mtMapThemeData.DissolveTopoName)  'mtMapThemeData.DissolveTopoName
					...
					ElseIf rdbSource.Checked Then
						...
						zzPaintAllLots(iTopoPurpose, tPaintLayerDef, tPLineLayerDef, Me.chkLayerByLanduse.Checked, Me.chkBypassLine.Checked, mtMapThemeData.LineTopoName)

					ElseIf rdbClosedPgons.Checked Then

						zzPaintLotPgonset(tPaintLayerDef, Me.chkLayerByLanduse.Checked)
					End If
				ElseIf mtMapThemeData.MapThemeID = DMAcadExt.enMapTheme.Parcels Then
					zzPaintAllParcels(tPaintLayerDef, Me.chkLayerByLanduse.Checked)
				ElseIf mtMapThemeData.MapThemeID = DMAcadExt.enMapTheme.Expropriation Then
					zzPaintExpro(tPaintLayerDef, Me.chkLayerByLanduse.Checked)
   ...
   Private Sub zzPaintAllLots(ByVal iTopoPurpose As DMAcadExt.enTopoPurpose, ByVal tPaintLayerDef As DMAcadExt.AcadLayerDef, ByVal tPLineLayerDef As DMAcadExt.AcadLayerDef, ByVal bLayerByLanduseID As Boolean, ByVal bBypassLine As Boolean, Optional sTopoName As String = Nothing)
      ...
      Dim sPLineLayer As String = String.Empty

      Dim oTopoScheme As TopoManager.TopoScheme.tsTopology = Nothing
      Dim bCurrentPaintLayerOK As Boolean
      Dim bCurrentPLineLayerOK As Boolean
		Dim bAllLots As Boolean = Not Me.chkLotsInOnly.Checked
		Dim iPolygonIndex As Integer
      Dim iResPaint As DMAcadExt.PaintException
      If dicLots Is Nothing Then
         zzLoadGraph()
         dicLots = TopoManager.TPlanGraph.TplnProject.Lots(iTopoPurpose)
      End If
      '  If bBypassLine OrElse True Then
      ...
      If dicLots IsNot Nothing Then
         ...
         Me.prbPaint.Value = 0
			For Each oLot As TopoManager.TPlanGraph.TplnLot In dicLots.Values
				...
				If iPolygonIndex < -999999 Then
					DMCommon.Debug.MsgBox("04_466", mtTopoSelection.IndexCount, mdicColorSchemes.Count, oLot.LanduseID, tColorScheme.ID, oLot.TopoID, oLot.Name)
				End If
				If bAllLots OrElse oLot.InPlan Then
					...
					If mdicColorSchemes.TryGetValue(oLot.LanduseID, tColorScheme) AndAlso tColorScheme.ID <> 0 Then
						If mtTopoSelection.Contains(iPolygonIndex, oLot.TopoID) Then
							...
							If bLayerByLanduseID Then
								...
								bCurrentPaintLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(tPaintLayerDef, True, False, False, False, oLot.LanduseID)
								bCurrentPLineLayerOK = DMAcadExt.AcadTransaction.CreateLayer(tPLineLayerDef, False, oLot.LanduseID)


								sPaintLayer = tPaintLayerDef.NamePlusExtension(oLot.LanduseID)
								sPLineLayer = tPLineLayerDef.NamePlusExtension(oLot.LanduseID)
							Else
								bCurrentPaintLayerOK = True
								sPLineLayer = tPLineLayerDef.Name
								bCurrentPLineLayerOK = Not String.IsNullOrEmpty(sPLineLayer)
							End If
							If bCurrentPaintLayerOK AndAlso bCurrentPLineLayerOK AndAlso (bBypassLine OrElse tColorScheme.NeedTopology) AndAlso (oTopoScheme IsNot Nothing) Then
								oLot.BorderAcObjID = oTopoScheme.CreatePgonDBPolyline(oLot.TopoID)
								DMAcadExt.AcadTransaction.SetLayer(oLot.BorderAcObjID, sPLineLayer)
								'  DMCommon.Debug.MsgBox("!08_177", oLot.BorderAcObjID, sPLineLayer)
							End If
							'    DMCommon.Debug.MsgBox("08_102a", oLot.BorderAcObjID)
							Dim tColorZebra As DMAcadExt.ColorZebra = tColorScheme.Zebra
							Dim tStrip As DMAcadExt.ColorStrip
							For i As Integer = 0 To tColorZebra.StripUB
								tStrip = tColorZebra.Strip(i)
								DMCommon.Debug.ExcelLog.SetNextValue(0, "Strip", tStrip.Width, tStrip.Color.AcadColor)
							Next
							iResPaint = oLot.Paint(DMAcadExt.PaintMethod.ColorScheme, tColorScheme, True, sPaintLayer)
							If Not bBypassLine AndAlso Not oLot.BorderAcObjID.IsNull Then
								DMAcadExt.AcadTransaction.EraseDBObject(oLot.BorderAcObjID)
								oLot.BorderAcObjID = Autodesk.AutoCAD.DatabaseServices.ObjectId.Null
							End If
							If iResPaint <> DMAcadExt.PaintException.OK Then
								Exit For
							End If
							prbPaint.Increment(1)
						End If
					ElseIf oLot.LanduseID <> 0 Then
      ...
      End If
   End Sub
   Private Sub zzPaintByLanduseTopo(ByVal iTopoPurpose As DMAcadExt.enTopoPurpose, ByVal tPaintLayerDef As DMAcadExt.AcadLayerDef, ByVal tPLineLayerDef As DMAcadExt.AcadLayerDef, ByVal bLayerByLanduseID As Boolean, ByVal bBypassLine As Boolean, Optional sTopoName As String = Nothing)
               ...
               If mdicColorSchemes.TryGetValue(oLusePgon.LanduseID, tColorScheme) AndAlso tColorScheme.ID <> 0 Then
                  If mtTopoSelection.Contains(iPolygonIndex, oLusePgon.TopoID) Then
                     ...
                     If bLayerByLanduseID Then
                        ...
                        bCurrentPaintLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(tPaintLayerDef, True, False, False, False, oLusePgon.LanduseID)
                        bCurrentPLineLayerOK = DMAcadExt.AcadTransaction.CreateLayer(tPLineLayerDef, False, oLusePgon.LanduseID)


                        sPaintLayer = tPaintLayerDef.NamePlusExtension(oLusePgon.LanduseID)
                        sPLineLayer = tPLineLayerDef.NamePlusExtension(oLusePgon.LanduseID)


                        'bCurrentLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(tPaintLayerDef, True, False, False, False, oLusePgon.LanduseID)
                        'sPaintLayer = tPaintLayerDef.NamePlusExtension(oLusePgon.LanduseID)
                     Else
                        bCurrentPaintLayerOK = True
                        sPLineLayer = tPLineLayerDef.Name
                     ...
                     '    DMCommon.Debug.MsgBox("08_199", True, sPLineLayer)

                     '  DMCommon.Debug.MsgBox("08_319", True, oLusePgon.TopoID, iPolygonIndex, bCurrentPaintLayerOK, bCurrentPLineLayerOK, bBypassLine, (oTopoScheme IsNot Nothing))

                     If bCurrentPaintLayerOK AndAlso bCurrentPLineLayerOK AndAlso (bBypassLine OrElse tColorScheme.NeedTopology) AndAlso (oTopoScheme IsNot Nothing) Then
                        oLusePgon.BorderAcObjID = oTopoScheme.CreatePgonDBPolyline(oLusePgon.TopoID)
                        DMAcadExt.AcadTransaction.SetLayer(oLusePgon.BorderAcObjID, sPLineLayer)
                     ...
                     If oLusePgon.Paint(DMAcadExt.PaintMethod.ColorScheme, tColorScheme, True, sPaintLayer, False) <> DMAcadExt.PaintException.OK Then
                        ...
                        Exit For
                     End If
                     If Not bBypassLine AndAlso Not oLusePgon.BorderAcObjID.IsNull Then
                        DMAcadExt.AcadTransaction.EraseDBObject(oLusePgon.BorderAcObjID)
                        oLusePgon.BorderAcObjID = Autodesk.AutoCAD.DatabaseServices.ObjectId.Null
   ...
   End Sub
	...
	Private Sub zzPaintExpro(ByVal tLayerDef As DMAcadExt.AcadLayerDef, ByVal bLayerByLanduseID As Boolean)
		...
		DMCommon.Debug.MsgBox("08_561i", tLayerDef.Name, DMCommon.Debug.ColCount(dicExpros))
		If dicExpros IsNot Nothing Then
			'	DMCommon.ExcelLog.SetEnumerable(0, "Keys", dicExpros.Keys)
			'	DMCommon.ExcelLog.SetEnumerable(0, "Values", dicExpros.Values)
			For Each oKeyValuePairy As KeyValuePair(Of Integer, TopoManager.TPlanGraph.TplnExpro) In dicExpros
				DMCommon.Debug.ExcelLog.SetNextValue(0, "DicEl", oKeyValuePairy.Key, oKeyValuePairy.Value.ExproTypeID, oKeyValuePairy.Value.ExproTypeName)
			Next
			Dim tColorScheme As DMAcadExt.ColorScheme = Nothing
			Me.prbPaint.Maximum = dicExpros.Count
			Me.prbPaint.Step = 1

			For Each oExpro As TopoManager.TPlanGraph.TplnExpro In dicExpros.Values
				'tColorScheme = mdicColorSchemes.Item(oLot.LanduseID)
				'MessageBox.Show(CStr(mdicColorSchemes.Count) & vbCrLf & CStr(oLot.LanduseID) & ";" & CStr(oLot.Name), "04_466")
				If mdicColorSchemes.TryGetValue(oExpro.ExproTypeID, tColorScheme) AndAlso tColorScheme.ID <> 0 Then
					If bLayerByLanduseID Then
						'  DMAcadExt.AcadTransaction.CreateLayer(tLayerDef, True, oParcel.Owner)

						bCurrentLayerOK = DMAcadExt.AcadTransaction.SetCurrentLayer(tLayerDef, True, False, False, False, oExpro.ExproTypeID)

						sPaintLayer = tLayerDef.NamePlusExtension(oExpro.ExproTypeID)
					End If

					If oExpro.Paint(DMAcadExt.PaintMethod.ColorScheme, tColorScheme, True, sPaintLayer) <> DMAcadExt.PaintException.OK Then
						Exit For
					End If
				End If
			Next
	...
	Private Sub zzFillColorSchemesDic()
		...
		TopoManager.TPlanGraph.TplnLanduse.FillColorSchemesDic(mtMapThemeData.MapThemeID, mdicColorSchemes, dScale)
		'DMCommon.Debug.MsgBox("05_276", mtMapThemeData.MapThemeID, mdicColorSchemes.Count, dScale)
	End Sub
   ...
   End Sub
	Private Sub cmdColorSchemeEditor_Click(sender As System.Object, e As System.EventArgs) Handles cmdColorSchemeEditor.Click
		Dim fColorEditor As frmColorSchemeEditor = New frmColorSchemeEditor(mtMapThemeData.MapThemeID, enColorEditorMode.Landuse, False)
		Dim oAcadWin As DMAcadExt.AcadWin = New DMAcadExt.AcadWin(Autodesk.AutoCAD.ApplicationServices.Application.MainWindow.Handle)
   ...
   Private Sub zzCheckDissolveTopo(ByVal bMsgBox As Boolean)
      ...
      DMAcadExt.AcadTransaction.Start()
      Dim tTopoRes As DMAcadExt.TopoRes = TopoManager.TopoCreator.CheckTopo(mtMapThemeData.DissolveTopoName, False, bMsgBox, mtMapThemeData.MapThemeID)
      DMAcadExt.AcadTransaction.Terminate()
      DMAcadExt.AcadDocument.Unlock()
   ...
   Private Sub zzCreatePgonSet()
		...
		'DMCommon.Debug.MsgBox("07_001s", mtMapThemeData.MapThemeID, mtMapThemeData.TopoPurpose, mtMapThemeData.GraphType, mtMapThemeData.TopoName, mtMapThemeData.ClosedPgonsLayers)
      ...
      moPgonSet.CalculateNewF(False)
		If moPgonSet.TopoPolygons.Count > 0 Then
			'	DMCommon.Debug.MsgBox("22_001", moPgonSet.TopoPolygons.Count)
			...
			Me.txtToNumber.Text = moPgonSet.TopoPolygons.Count.ToString()
		End If
   ...
   Private Sub zzPaintLotPgonset(ByVal tLayerDef As DMAcadExt.AcadLayerDef, ByVal bLayerByLanduseID As Boolean)
		'	DMCommon.Debug.MsgBox("21_323", mtMapThemeData.GraphType, CStr(moPgonSet IsNot Nothing))
		Dim iPaintException As DMAcadExt.PaintException = DMAcadExt.PaintException.OK
		...
		If mtMapThemeData.GraphType = DMAcadExt.enGraphType.ClosedPolygons Then
         ...
         If moPgonSet IsNot Nothing Then
            ...
            sPaintLayer = TopoManager.TPlanGraph.TplnProject.SetPaintLayers(mtMapThemeData.MapThemeID, DMAcadExt.enTopoPurpose.Approved)
				...
				'DMCommon.Debug.MsgBox("!dicTopoPolygons.Count", dicTopoPolygons.Count)
               ...
               DMAcadExt.AcadDocument.Counter = iPolygonIndex
					...
					If mdicColorSchemes.TryGetValue(oLot.LanduseID, tColorScheme) AndAlso tColorScheme.ID <> 0 Then
						If mtTopoSelection.Contains(iPolygonIndex, oLot.ExteriorHandle.Value) Then
							...
							End If
							iPaintException = oLot.Paint(DMAcadExt.PaintMethod.ColorScheme, tColorScheme, True, sPaintLayer)

							If iPaintException <> DMAcadExt.PaintException.OK Then
								DMCommon.Debug.MsgBox("!PaintExc", iPolygonIndex, oLot.Name, oLot.LanduseID, iPaintException)
								'Exit For
							End If
						End If
					Else
						DMCommon.Debug.MsgBoxLoop("!NotExists", oLot.LanduseID)
						If sLanduseNotFoundList.Length <> 0 Then
                     sLanduseNotFoundList &= ","
                  End If
                  sLanduseNotFoundList &= CStr(oLot.LanduseID)
   ...
   End Sub
	...
	Private Sub rdbClosedPgons_CheckedChanged(sender As Object, e As EventArgs) Handles rdbClosedPgons.CheckedChanged

	End Sub

	Private Sub txtToNumber_TextChanged(sender As Object, e As EventArgs) Handles txtToNumber.TextChanged

	End Sub

	Private Sub txtSourceClosePgonCount_TextChanged(sender As Object, e As EventArgs) Handles txtSourceClosePgonCount.TextChanged
