...
Public Class frmHanitControl
   ...
   Private mfTextEditor As frmTextEditor
	Private mfHanitGeneral As TopoUI.frmHanitGeneral
	...
	Public Sub GeneralView(bView As Boolean)
		...
		If mfHanitGeneral IsNot Nothing AndAlso Not mfHanitGeneral.IsDisposed Then
			mfHanitGeneral.Visible = bView
		End If
	...
	Private Sub dgvStages_CellValueChanged(oSender As System.Object, e As DataGridViewCellEventArgs) Handles dgvStages.CellValueChanged
		If mbEventsEnabled AndAlso e.RowIndex >= 0 Then
			...
			If DMAcadExt.AcadDocument.IsLocked Then
				DMAcadExt.AcadTransaction.Start()
				'    DMAcadExt.AcadTransaction.OpenModelSpace(Autodesk.AutoCAD.DatabaseServices.OpenMode.ForWrite, False)
				Dim oDynObjecct As System.Object = dgvStages.Rows(e.RowIndex).Cells.Item("cchLayerOn").Value
	...
	Private Sub cmdOpenGen_Click(oSender As System.Object, e As EventArgs) Handles cmdOpenGen.Click
		...
		If mfHanitGeneral Is Nothing OrElse mfHanitGeneral.IsDisposed Then
			mfHanitGeneral = New TopoUI.frmHanitGeneral(True)
		End If
		mfHanitGeneral.GushName = TopoManager.TPlanGraph.UD_ParcelKey.BaseBlockName
		mfHanitGeneral.NormalParcels = sNormalParcelsString
		mfHanitGeneral.TempParcels = sTempParcelsString

		mfHanitGeneral.SetPlanType(bIsTTTG)
