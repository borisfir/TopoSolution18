...
Public Enum enColorEditorMode
	...
	ExproAAA
End Enum
Public Class frmColorSchemeEditor
	...
	Private mbAllowAddToSet As Boolean
	Private miMapThemeID As DMAcadExt.enMapTheme
	Private msColorSchemeTable As String
	'	Private miColorSchemeID As Integer
	...
	Public Event AddLanduse(iLanduseID As Integer, iColoSchemeID As Integer)

	Private Sub zzMyInitializeComponent()
		Const sLuseElementName As String = "יעוד"
		Const sExproElementName As String = "סוג הפקעה"
		...
		'	System.Windows.Forms.MessageBox.Show(miFormMode.ToString(), "26_901")

		Select Case miMapThemeID
			Case DMAcadExt.enMapTheme.LotApproved, DMAcadExt.enMapTheme.LotProposed
				sElementName = sLuseElementName
				msColorSchemeTable = "ColorSchemes"
			Case DMAcadExt.enMapTheme.Expropriation
				sElementName = sExproElementName
				msColorSchemeTable = "ColorSchemesExpro"
				Me.chkStandard.Visible = False
			Case Else
				sElementName = String.Empty
		End Select


		Me.cmbColorSchemes.DataSource = LanduseData.GetMainTable(miMapThemeID)
		...
		If miFormMode = enColorEditorMode.Landuse Then
			Me.Text = "צביעת " & " " & sElementName
			Me.Text = "סכימות צביעה"
			Me.Label2.Text = "קוד " & sElementName
			Me.Label3.Text = "שם " & sElementName
	...
	End Sub
	Public Property AllowAddToSet As Boolean
		Get
			Return mbAllowAddToSet
		End Get
		Set(bValue As Boolean)
			mbAllowAddToSet = bValue
			tsbAddToSet.Enabled = mbAllowAddToSet
		End Set
	End Property
	...
	Public Sub New(iMapThemeID As DMAcadExt.enMapTheme, ByVal iMode As enColorEditorMode, ByVal bReadOnly As Boolean, Optional ByVal iLanduseID As Integer = 0, Optional ByVal iColorSchemeID As Integer = 0)
		...
		'DMCommon.Debug.MsgBox("08_422g", miFormMode, miLanduseID, miColorSchemeID, Me.cmbColorSchemes.Visible)
	...
	Private Function zzGetElementTableName() As String
		Select Case miMapThemeID
			Case DMAcadExt.enMapTheme.LotApproved, DMAcadExt.enMapTheme.LotProposed
				Return "Landuse_1F"
			Case DMAcadExt.enMapTheme.Expropriation
	...
	Private Function zzGetNewColorSchemeID(ByVal bLanduse As Boolean) As Integer
		Dim sCom As String = "SELECT Max(ID) FROM " & msColorSchemeTable & " WHERE"
		Dim sMin, sMax As String
		If bLanduse Then
			Select Case miMapThemeID
				Case DMAcadExt.enMapTheme.LotApproved, DMAcadExt.enMapTheme.LotProposed
					sMin = Convert.ToString(TopoManager.TPlanGraph.TplnProject.Landuse_ID_Control.MinID)
					sMax = Convert.ToString(TopoManager.TPlanGraph.TplnProject.Landuse_ID_Control.MaxID)
				Case DMAcadExt.enMapTheme.Expropriation
					sMin = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ExproType_ID_Control.MinID)
					sMax = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ExproType_ID_Control.MaxID)
				Case Else
					sMin = "1"
					sMax = "1000"
			End Select
		...
		Else
			...
			Select Case miMapThemeID
				Case DMAcadExt.enMapTheme.LotApproved, DMAcadExt.enMapTheme.LotProposed
					sMin = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ColorScheme_ID_Control.MinID)
					sMax = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ColorScheme_ID_Control.MaxID)
				Case DMAcadExt.enMapTheme.Expropriation
					sMin = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ColorSchemeExpro_ID_Control.MinID)
					sMax = Convert.ToString(TopoManager.TPlanGraph.TplnProject.ColorSchemeExpro_ID_Control.MaxID)
				Case Else
					sMin = "1"
					sMax = "1000"
			End Select
		...
		If sMax.Length <> 0 Then
			sCom &= " AND (ID <= " & sMax & ")"
		End If
		DMCommon.Debug.MsgBox("13_201h", sCom, sMin, sMax)
		...
		If oRes Is Nothing Then
			System.Windows.Forms.MessageBox.Show(sCom & vbCrLf & CStr(DMAcadExt.ColorScheme.MaxStandardID + 1), "2_0112")
			Return 2001
		ElseIf IsDBNull(oRes) Then
			If bLanduse Then
				System.Windows.Forms.MessageBox.Show(sCom & vbCrLf & CStr(DMAcadExt.ColorScheme.MaxStandardID + 1), "2_0113")
				Return TopoManager.TPlanGraph.TplnProject.Landuse_ID_Control.MinID
			Else
				System.Windows.Forms.MessageBox.Show(sCom & vbCrLf & CStr(DMAcadExt.ColorScheme.MaxStandardID + 1), "2_0114")
				Return TopoManager.TPlanGraph.TplnProject.ColorScheme_ID_Control.MinID()
			End If
		Else
			Try
				System.Windows.Forms.MessageBox.Show(sCom & vbCrLf & CStr(DirectCast(oRes, Integer) + 1), "2_0115")
		...
		If mtColorScheme.HasName Then
			If miLanduseID = 0 And miFormMode = enColorEditorMode.Landuse Then
				...
				Try
					...
					End If
					DMCommon.Debug.MsgBox("13_217e", miFormMode, miNewID)
				Catch oEx As Exception
			...
			End If
			'	MessageBox.Show(CStr(mtColorSchemeDB.ID) & ":" & miFormMode.ToString() & vbCrLf & CStr(miNewID), "18_011")
			If miNewID <> 0 And miFormMode = enColorEditorMode.Landuse Then
				'DMCommon.Debug.MsgBox("13_438a", CStr(miNewID) & ":" & mtColorScheme.Name)
				bRes = TPlanGraph.TplnProject.AddNewLanduse(miNewID, mtColorScheme.Name)
				'DMCommon.Debug.MsgBox("13_438d", CStr(miNewID) & ":" & mtColorScheme.Name)
				bRes = TPlanGraph.TplnProject.AddNewElement(zzGetElementTableName(), miNewID, mtColorScheme.Name)
				'DMCommon.Debug.MsgBox("13_438e", CStr(miNewID) & ":" & mtColorScheme.Name)
			End If
			'MessageBox.Show(CStr(mtColorSchemeDB.ID) & ":" & miFormMode.ToString(), "18_012")
			Try
	...
	Private Sub zzUpdate()
		...
		DMCommon.Debug.MsgBox("13_217c", mtColorSchemeDB.ID, mtColorScheme.ID, miFormMode)
		If (miFormMode = enColorEditorMode.Landuse AndAlso miLanduseID <> 0) OrElse (miFormMode = enColorEditorMode.ColorScheme AndAlso miLanduseID <> 0) Then
			...
			bPaint = True
		End If
		'	MessageBox.Show(CStr(mtColorSchemeDB.ID) & ":" & miFormMode.ToString(), "18_002")
		If miFormMode = enColorEditorMode.Landuse AndAlso miLanduseID <> 0 Then
			...
			End If
		End If
		DMCommon.Debug.MsgBox("13_217d", bRes)
		If bPaint Then
			pcbPicture.Invalidate()
			'	zzPaint(Nothing)
		End If
   ...
   End Sub
	Private Sub zzBuildColorScheme()
		...
		mtColorScheme.TableName = msColorSchemeTable
		...
		Try
			...
			With tHatch
				...
				If Not String.IsNullOrEmpty(sPatternName) Then
	...
	Private Sub zzDisplayColorScheme()
		...
		End If
		DMCommon.Debug.MsgBox("13_200w", miFormMode)
	...
	End Sub
	Private Sub zzAddToSet()
		Dim iColorSchemeID As Integer
		Dim bRes As Boolean
		zzBuildColorScheme()

		If mtColorSchemeDB = mtColorScheme Then
			iColorSchemeID = miLanduseID
			bRes = True
		Else
			Dim iNewColorSchemeID As Integer = Me.zzGetNewColorSchemeID(True)
			bRes = mtColorScheme.AddNewToDB(iNewColorSchemeID)
		End If
		If bRes Then
			RaiseEvent AddLanduse(miLanduseID, miLanduseID)
		Else
			MessageBox.Show(CStr(bRes) & ":Error", "01_138")
		End If
	...
	Private Sub zzSetBySelect()
		...
		mtColorSchemeDB = New DMAcadExt.ColorScheme(miLanduseID, 2.0, msColorSchemeTable)
	...
	Private Sub zzSetColor(ByRef oTextBox As TextBox, ByVal tDMColor As DMAcadExt.DMColor)	', ByVal bAnyway As Boolean
		With oTextBox
			...
			Catch oEx As Exception
				System.Windows.Forms.MessageBox.Show(oEx.Message & vbCrLf & tDMColor.ARGBString, "frColorSchemeEditor - zzSetColor")
			End Try
	...
	Private Class LanduseData
		...
		End Function
		Public Shared Function GetMainTable(iMapThemeID As DMAcadExt.enMapTheme) As DataTable
			Dim sComText As String
			Select Case iMapThemeID
				Case DMAcadExt.enMapTheme.LotApproved, DMAcadExt.enMapTheme.LotProposed
					sComText = zzGetMainTableComText()
				Case DMAcadExt.enMapTheme.Expropriation
					sComText = zzGetExproTableComText()
				Case Else
					sComText = Nothing
			End Select

			DMCommon.Debug.MsgBox("13_200t", sComText)
		...
		Private Shared Function zzGetMainTableComText() As String
			...
			Select Case TPlServerDB.ServerDB.CurrentServerDB.Provider
				Case TPlServerDB.TPlProvider.ProviderJet
					Return "SELECT ID,ID & ' - ' & Name AS Name FROM Landuses_1F WHERE ID<200000 ORDER BY ID"
				Case TPlServerDB.TPlProvider.ProviderSQLServer
					Return "SELECT ID,ID_Name AS Name FROM Landuses_1F WHERE ID<200000 ORDER BY ID"
				Case Else
					Return String.Empty
			End Select

		End Function
		Private Shared Function zzGetExproTableComText() As String
			Dim sName As String = "Name"
			Select Case TPlServerDB.ServerDB.CurrentServerDB.Provider
				Case TPlServerDB.TPlProvider.ProviderJet
					Return "SELECT ID,ID & ' - ' & Name AS Name FROM ExproTypes"
				Case TPlServerDB.TPlProvider.ProviderSQLServer
					Return "SELECT ID,ID_Name AS Name FROM ExproTypes"
				Case Else
					Return String.Empty
			End Select
	...
	Private Sub tsbAddToSet_Click(sender As System.Object, e As System.EventArgs) Handles tsbAddToSet.Click
   ...
   Private Sub tsbWinColor_Click(oSender As System.Object, e As EventArgs) Handles tsbWinColor.Click

   End Sub

	Private Sub frmColorSchemeEditor_Load(oSender As System.Object, e As EventArgs) Handles Me.Load
		mbEventsEnabled = True
