...
Public Class Form3
   ...
   End Sub
	Private Sub zzInitDB()
		Const sServerName As String = TPlServerDB.ServerDB.DBServerName
		...
		TPlServerDB.ServerDB.CurrentServerDB.SetSQL(sServerName, sDatabaseName, True, False)
   ...
   End Function
	...
	Private Sub Button8_Click(oSender As System.Object, e As EventArgs) Handles Button8.Click
		Dim oBalanceArea As TopoManager.BalanceArea
		'	Dim daResultArea() As Double = {495, 15500}
		'Dim daResultArea() As Double = {92.6355727, 58.05938375}
		Dim daResultArea() As Double = {34096.5849592964, 4825.01546053131, 922.64711072499}




		Dim daRangeMin() As Double = {33872.0118484598, 4760.2989270181, 1.0}
		Dim daRangeMax() As Double = {34323.7839162333, 4890.74403235115, 1000000.0}


		'	Dim daRangeMin() As Double = {469.56, 15099.28}
		'Dim daRangeMax() As Double = {525.12, 15918.89}
		'	Dim daRangeMin() As Double = {0.0, 0.0}
		'	Dim daRangeMax() As Double = {0.0, 0.0}
		'	Dim dSourceArea As Double = 15851.0
		'Dim dSourceArea As Double = 163.0
		Dim dSourceArea As Double = 40213.0
		Dim dResAreaNorm(2) As Double
		Dim dResArea(2) As Double

		'Dim dSourceArea As Double = 15861.0
		'Before Range
		oBalanceArea = New TopoManager.BalanceArea(daResultArea, 1.0, dSourceArea, True, "Ud")
		dResAreaNorm(0) = oBalanceArea.OutputItemFloat(0)
		dResAreaNorm(1) = oBalanceArea.OutputItemFloat(1)
		dResAreaNorm(2) = oBalanceArea.OutputItemFloat(2)


		'After Range
		Dim tArea As TopoManager.TPlanGraph.ParcelArea
		Dim dRangeMin, dRangeMax As Double

		tArea = New TopoManager.TPlanGraph.ParcelArea()
		tArea.CalculateArea(0.0, daResultArea(0), 0)
		tArea.CalculateRangeNewPlus(0, 0, dRangeMin, dRangeMax)
		daRangeMin(0) = dRangeMin
		daRangeMax(0) = dRangeMax

		tArea = New TopoManager.TPlanGraph.ParcelArea()
		tArea.CalculateArea(0.0, daResultArea(1), 0)
		tArea.CalculateRange(dRangeMin, dRangeMax)
		daRangeMin(1) = dRangeMin
		daRangeMax(1) = dRangeMax

		daRangeMin(2) = 0
		daRangeMax(2) = 1000000

		oBalanceArea = New TopoManager.BalanceArea(daResultArea, daRangeMin, daRangeMax, 1.0, dSourceArea, True, "Ud")
		dResArea(0) = oBalanceArea.OutputItemFloat(0)
		dResArea(1) = oBalanceArea.OutputItemFloat(1)
		dResArea(2) = oBalanceArea.OutputItemFloat(2)
